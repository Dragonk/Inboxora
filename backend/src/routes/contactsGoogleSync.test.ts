...
const mocks = vi.hoisted(() => ({ query: vi.fn(), syncGoogleContacts: vi.fn(), configured: { value: true }, operational: { value: true } }));
...
describe('POST /api/contacts/providers/google/sync', () => {
  it('refuses an existing Google connection before any query or provider call when the API is disabled', async () => {
