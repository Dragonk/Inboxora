...
const wrap = (lines: string[]): string => ['BEGIN:VCALENDAR', 'VERSION:2.0', 'BEGIN:VEVENT', 'UID:event-1',
...
describe('writing descriptions back to iCalendar', () => {
  ...
  it('round-trips a sanitized HTML description through the parser', () => {
    ...
    expect(mustParse(wrap(descriptionContentLines(html, escape))).description).toBe('<p>Plan &amp; details</p><p>Next</p>');
  });
  ...
  it('reads the HTML alternative of an invitation accepted from mail', () => {
    ...
    expect(mustParse(raw).description).toBe('<p>Mail <strong>body</strong></p>');
  });
});
