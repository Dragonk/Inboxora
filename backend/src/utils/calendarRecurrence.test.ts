...
function mustParse(raw: string): ParsedICalendarEvent {
  const event = parseCalendarEvent(raw);
  if (!event) throw new Error('expected the fixture to parse');
  return event;
}

function startIso(event: ProjectedEvent): string {
  if (!event.starts_at) throw new Error('expected a projected start date');
  return event.starts_at.toISOString();
}

function findEvent(events: ProjectedEvent[], summary: string): ProjectedEvent {
  const found = events.find(event => event.summary === summary);
  if (!found) throw new Error('expected the projected event');
  return found;
}

function mustTruncate(raw: string, recurrenceId: string): { raw: string; empty: boolean } {
  const result = truncateSeriesBefore(raw, recurrenceId);
  if (!result) throw new Error('expected the series to truncate');
  return result;
}

const resource = (extra: string = '', exception: string = '') => ['BEGIN:VCALENDAR', 'VERSION:2.0', 'BEGIN:VEVENT', 'UID:series', 'DTSTART;TZID=Europe/Warsaw:20260322T090000', 'DTEND;TZID=Europe/Warsaw:20260322T100000', 'RRULE:FREQ=WEEKLY;COUNT=4', 'SUMMARY:Weekly', 'DESCRIPTION:Full agenda', extra, 'END:VEVENT', exception, 'END:VCALENDAR'].filter(Boolean).join('\r\n');
const row = (raw: string) => ({ id: 'row', calendar_id: 'cal', raw_ical: raw });
...
describe('recurring calendar resources', () => {
 it('expands a series across DST while preserving wall time and descriptions', () => {
   ...
   expect(events.map(startIso)).toEqual(['2026-03-22T08:00:00.000Z', '2026-03-29T07:00:00.000Z', '2026-04-05T07:00:00.000Z', '2026-04-12T07:00:00.000Z']);
 ...
 it('honors excluded dates and moved exceptions, grouping by UID on import', () => {
   ...
   expect(startIso(findEvent(events, 'Moved'))).toBe('2026-04-06T09:00:00.000Z');
 });
...
describe('cancelling a series from an occurrence onward', () => {
  const seriesWith = (extra: string[] = []) => ['BEGIN:VCALENDAR', 'VERSION:2.0', 'BEGIN:VEVENT', 'UID:s',
  ...
  const starts = (raw: string) => projectCalendarResource(row(raw), new Date('2026-01-01'), new Date('2026-03-01'))
    .map(event => startIso(event).slice(5, 16));
  ...
  it('reports that cutting at the first occurrence leaves nothing', () => {
    const result = mustTruncate(seriesWith(), '2026-01-05T09:00:00');
  ...
  it('keeps an earlier moved instance and drops the ones past the cut', () => {
    ...
    const result = mustTruncate(seriesWith([...earlier, ...later]), '2026-01-09T09:00:00');
  ...
  it('handles a date-valued series without leaving the boundary occurrence behind', () => {
    ...
    const result = mustTruncate(raw, '2026-01-09');
    expect(projectCalendarResource(row(result.raw), new Date('2026-01-01'), new Date('2026-03-01')).map(event => startIso(event).slice(5, 10)))
