...
async function matchingLegacyLogicalRows(client, hydrated) {
  ...
  const result = await client.query(`
    SELECT lm.id, lm.conversation_id, lm.message_id_collision_key, lm.created_at,
           sample.message_id, sample.date, sample.subject, sample.from_email,
           lm.parent_logical_message_id, sample.in_reply_to, sample.thread_references
      FROM logical_messages lm
      LEFT JOIN LATERAL (
        SELECT m.message_id, m.date, m.subject, m.from_email, m.in_reply_to, m.thread_references
          FROM messages m
         WHERE m.logical_message_id = lm.id AND m.is_deleted = false
         ORDER BY m.date ASC NULLS LAST, m.id
         LIMIT 1
      ) sample ON TRUE
     WHERE lm.user_id = $1 AND lm.account_id = $3 AND lm.canonical_message_id = $2
     ORDER BY lm.created_at ASC, lm.id
     FOR UPDATE OF lm
...
async function consolidateLegacyLogicalRows(client, hydrated, rows, preferredId = null) {
  ...
  const protectedState = await client.query(`
    SELECT (
      EXISTS (
        SELECT 1 FROM conversation_overrides
         WHERE user_id = $1 AND account_id = $4
           AND (logical_message_id = ANY($2::uuid[]) OR conversation_id = ANY($3::uuid[]))
      ) OR EXISTS (
        SELECT 1 FROM conversations
         WHERE user_id = $1 AND account_id = $4 AND id = ANY($3::uuid[]) AND manually_locked = true
      )
    ) AS protected
  `, [hydrated.userId, ids, conversationIds, hydrated.accountId]);
  if (protectedState.rows[0]?.protected) return preferred;

  const winner = preferred;
  const losers = uniqueRows.filter(row => row.id !== winner.id);
  const loserIds = losers.map(row => row.id);
  if (loserIds.length) {
    // Repair graph references before deleting duplicate identities. Child reference
    // rows are copied with conflict handling because two legacy duplicates can carry
    // the same unresolved RFC edge.
    await client.query(`
      INSERT INTO unresolved_message_references
        (user_id, account_id, child_logical_message_id, referenced_message_id, relation_type,
         reference_position, resolved_logical_message_id, resolved_at, created_at)
      SELECT user_id, account_id, $1, referenced_message_id, relation_type, reference_position,
             CASE WHEN resolved_logical_message_id = ANY($2::uuid[]) THEN $1 ELSE resolved_logical_message_id END,
             resolved_at, created_at
        FROM unresolved_message_references
       WHERE user_id = $3 AND account_id = $4 AND child_logical_message_id = ANY($2::uuid[])
      ON CONFLICT DO NOTHING
    `, [winner.id, loserIds, hydrated.userId, hydrated.accountId]);
    await client.query('DELETE FROM unresolved_message_references WHERE user_id = $1 AND account_id = $3 AND child_logical_message_id = ANY($2::uuid[])', [hydrated.userId, loserIds, hydrated.accountId]);
    await client.query('UPDATE unresolved_message_references SET resolved_logical_message_id = $1 WHERE user_id = $2 AND account_id = $4 AND resolved_logical_message_id = ANY($3::uuid[])', [winner.id, hydrated.userId, loserIds, hydrated.accountId]);

    await client.query(`
      INSERT INTO conversation_evidence
        (user_id, account_id, conversation_id, logical_message_id, evidence_type,
         evidence_value_hash, weight, algorithm_version, details, created_at)
      SELECT user_id, account_id, COALESCE($1, conversation_id), $2, evidence_type,
             evidence_value_hash, weight, algorithm_version, details, created_at
        FROM conversation_evidence
       WHERE user_id = $3 AND account_id = $5 AND logical_message_id = ANY($4::uuid[])
      ON CONFLICT DO NOTHING
    `, [winner.conversation_id, winner.id, hydrated.userId, loserIds, hydrated.accountId]);
    await client.query('DELETE FROM conversation_evidence WHERE user_id = $1 AND account_id = $3 AND logical_message_id = ANY($2::uuid[])', [hydrated.userId, loserIds, hydrated.accountId]);
    ...
    await client.query(`
      UPDATE messages m
         SET logical_message_id = $1, conversation_id = $2, conversation_user_id = $3
       WHERE m.logical_message_id = ANY($4::uuid[])
         AND m.account_id = $5 AND EXISTS (SELECT 1 FROM email_accounts a WHERE a.id = m.account_id AND a.user_id = $3)
    `, [winner.id, winner.conversation_id, hydrated.userId, loserIds, hydrated.accountId]);
    await client.query('DELETE FROM logical_messages WHERE id = ANY($1::uuid[]) AND user_id = $2 AND account_id = $3', [loserIds, hydrated.userId, hydrated.accountId]);
  }
  await client.query('UPDATE logical_messages SET message_id_collision_key = $1, updated_at = NOW() WHERE id = $2 AND user_id = $3 AND account_id = $4', [hydrated.collisionKey, winner.id, hydrated.userId, hydrated.accountId]);
  for (const conversationId of conversationIds) await refreshConversationAggregates(client, hydrated.userId, conversationId);
  if (conversationIds.length) await client.query(`
  ...
  `, [hydrated.userId, conversationIds]);
  ...
  if (hydrated.canonicalMessageId) {
    ...
    const result = await client.query(
      `SELECT id, conversation_id, message_id_collision_key
        ...
        WHERE user_id = $1 AND account_id = $4 AND canonical_message_id = $2 AND message_id_collision_key = $3
        ORDER BY created_at ASC
        LIMIT 1
        FOR UPDATE`,
      [hydrated.userId, hydrated.canonicalMessageId, hydrated.collisionKey, hydrated.accountId],
    );
    if (result.rows.length > 0) {
      if (!repairExisting) return { logical: result.rows[0], collision: false };
      const legacyMatches = await matchingLegacyLogicalRows(client, hydrated);
      const matching = [result.rows[0], ...legacyMatches.filter(row => row.id !== result.rows[0].id)];
      return { logical: await consolidateLegacyLogicalRows(client, hydrated, matching, result.rows[0].id), collision: false };
    }
    // Identity-v1 rows used body-dependent keys. Recompute candidate identities
    // from their physical envelope and reuse/consolidate true matches before
    // treating another key as a genuine Message-ID collision.
    const legacyMatches = repairExisting ? await matchingLegacyLogicalRows(client, hydrated) : [];
    if (legacyMatches.length) return { logical: await consolidateLegacyLogicalRows(client, hydrated, legacyMatches), collision: false };
    ...
    const existing = await client.query(
      `SELECT 1 FROM logical_messages
        WHERE user_id = $1 AND account_id = $3 AND canonical_message_id = $2
        LIMIT 1`,
      [hydrated.userId, hydrated.canonicalMessageId, hydrated.accountId],
    );
  ...
  if (refs.length > 0) {
    ...
    const batch = await client.query(
      `SELECT id, conversation_id, canonical_message_id, subject FROM logical_messages
        WHERE user_id = $1 AND account_id = $3 AND canonical_message_id = ANY($2::text[])
        ORDER BY created_at ASC FOR UPDATE`,
      [hydrated.userId, reversed, hydrated.accountId],
    );
...
async function findPreviousSeriesMessage(client, hydrated) {
  ...
  const result = await client.query(`
    SELECT lm.*, c.logical_message_count, m.from_email, m.to_addresses, m.body_text, m.date, m.conversation_raw_headers
      FROM logical_messages lm
      JOIN conversations c ON c.id = lm.conversation_id AND c.user_id = lm.user_id AND c.account_id = lm.account_id
      JOIN messages m ON m.logical_message_id = lm.id AND m.account_id = lm.account_id
     WHERE lm.user_id = $1 AND lm.account_id = $3 AND lm.canonical_subject = $2
       AND lm.message_date > NOW() - INTERVAL '7 days'
       AND m.is_deleted = false
     ORDER BY lm.message_date DESC NULLS LAST LIMIT 10
  `, [hydrated.userId, hydrated.canonicalSubject, hydrated.accountId]);
...
export async function _upsertConversationCopyWithClient(client, copy, { identities = [], provider = null, userId = null, repairExisting = false } = {}) {
    ...
    if (!logical) {
      ...
      if (!logical) {
        const winner = hydrated.canonicalMessageId
          ? await client.query(`SELECT id, conversation_id FROM logical_messages WHERE user_id = $1 AND account_id = $4 AND canonical_message_id = $2 AND message_id_collision_key = $3 FOR UPDATE`, [hydrated.userId, hydrated.canonicalMessageId, hydrated.collisionKey, hydrated.accountId])
          : await client.query(`SELECT id, conversation_id FROM logical_messages WHERE user_id = $1 AND account_id = $4 AND canonical_message_id IS NULL AND body_fingerprint = $2 AND header_fingerprint = $3 ORDER BY created_at ASC LIMIT 1 FOR UPDATE`, [hydrated.userId, hydrated.bodyFingerprint, hydrated.headerFingerprint, hydrated.accountId]);
    ...
    // provider identity is next, and the existing logical assignment is only the
    // fallback. Manual overrides/locks are applied immediately below and can still
    // preserve or redirect explicit user intent.
    let conversationId = repairExisting
      ? parent?.conversation_id || providerConversationId || logical.conversation_id
      : logical.conversation_id || providerConversationId || parent?.conversation_id;
    ...
    // During repair, evaluate conversation-level intent against the message's current
    // placement before RFC/provider evidence proposes a different destination. Otherwise
    // a lock on the existing conversation would be invisible after conversationId was
    // tentatively replaced with the parent conversation.
    const overrideConversationId = repairExisting && logical.conversation_id
      ? logical.conversation_id
      : conversationId;
    const override = await effectiveConversationOverride(client, { userId: hydrated.userId, accountId: hydrated.accountId, conversationId: overrideConversationId, logicalMessageId: logical.id });
    if (override.merge?.target_id) requestedConversationId = await resolveConversationAlias(client, { userId: hydrated.userId, accountId: hydrated.accountId, conversationId: override.merge.target_id });
    // manual-split records the deliberate destination as its current conversation;
    // older rows do not necessarily carry target_id. Preserve that placement on replay.
    if (override.split) requestedConversationId = override.split.target_id
      ? await resolveConversationAlias(client, { userId: hydrated.userId, accountId: hydrated.accountId, conversationId: override.split.target_id })
      : logical.conversation_id;
    // A locked conversation preserves its existing placement even when replay
    // discovers a different RFC parent. Message-level split/move/include overrides
    // remain authoritative through requestedConversationId below.
    if (override.locked && logical.conversation_id) conversationId = logical.conversation_id;
    if (override.move?.target_id) requestedConversationId = await resolveConversationAlias(client, { userId: hydrated.userId, accountId: hydrated.accountId, conversationId: override.move.target_id });
    const previousConversationId = logical.conversation_id;
    ...
    if (override.forceExclude) {
      ...
      await client.query('UPDATE messages SET conversation_id = NULL, threading_reason = $1, threading_confidence = 0 WHERE logical_message_id = $2 AND conversation_user_id = $3 AND account_id = $4', ['manual-force-exclude', logical.id, hydrated.userId, hydrated.accountId]);
      await client.query('UPDATE logical_messages SET conversation_id = NULL, parent_logical_message_id = NULL, updated_at = NOW() WHERE id = $1 AND user_id = $2 AND account_id = $3', [logical.id, hydrated.userId, hydrated.accountId]);
      if (previousConversationId) await client.query('DELETE FROM conversation_evidence WHERE logical_message_id = $1 AND conversation_id = $2 AND user_id = $3 AND account_id = $4', [logical.id, previousConversationId, hydrated.userId, hydrated.accountId]);
      await client.query('UPDATE unresolved_message_references SET resolved_logical_message_id = NULL, resolved_at = NULL WHERE resolved_logical_message_id = $1 AND user_id = $2 AND account_id = $3', [logical.id, hydrated.userId, hydrated.accountId]);
    ...
    if (override.forceInclude?.target_id) requestedConversationId = await resolveConversationAlias(client, { userId: hydrated.userId, accountId: hydrated.accountId, conversationId: override.forceInclude.target_id });
    if (override.locked && conversationId) requestedConversationId = await resolveConversationAlias(client, { userId: hydrated.userId, accountId: hydrated.accountId, conversationId });
    ...
    await client.query(`UPDATE logical_messages SET conversation_id = $1, parent_logical_message_id = CASE WHEN $4::boolean THEN $2 ELSE COALESCE(parent_logical_message_id, $2) END, updated_at = NOW() WHERE id = $3 AND account_id = $5`, [conversationId, parent?.id || null, logical.id, repairExisting, hydrated.accountId]);
    const attached = await client.query(`UPDATE messages SET logical_message_id = $1, conversation_id = $2, conversation_user_id = $3, canonical_message_id = $4, provider_message_id = COALESCE($5, provider_message_id), provider_thread_id = COALESCE($6, provider_thread_id), provider_namespace = COALESCE($7, provider_namespace), thread_id = CASE WHEN $7 LIKE 'gmail:%' AND $6 IS NOT NULL THEN 'gmail:' || $6 ELSE thread_id END, threading_reason = $8, threading_confidence = $9, threading_algorithm_version = 'conversation-v2', row_version = row_version + 1 WHERE id = $10 RETURNING id`, [logical.id, conversationId, hydrated.userId, hydrated.canonicalMessageId, provider?.providerMessageId || null, provider?.providerThreadId || null, provider?.namespace || provider?.provider || null, decision.reason, decision.confidence, source.id]);
    if (attached.rowCount !== 1) throw new Error('Conversation copy attachment failed');
    if (conversationChanged) {
      // A LogicalMessage represents all physical copies of one RFC message. When
      // replay repairs its conversation, move every copy atomically rather than
      // leaving sibling account/folder copies attached to the stale container.
      await client.query('UPDATE messages SET conversation_id = $1, conversation_user_id = $2 WHERE logical_message_id = $3 AND conversation_user_id = $2 AND account_id = $4', [conversationId, hydrated.userId, logical.id, hydrated.accountId]);
      if (previousConversationId) await refreshConversationAggregates(client, hydrated.userId, previousConversationId);
      if (previousConversationId) await client.query(`
        DELETE FROM conversations c
         WHERE c.user_id = $1 AND c.id = $2
           AND NOT EXISTS (SELECT 1 FROM logical_messages lm WHERE lm.conversation_id = c.id)
           AND NOT EXISTS (SELECT 1 FROM conversation_aliases ca WHERE ca.alias_conversation_id = c.id OR ca.canonical_conversation_id = c.id)
           AND NOT EXISTS (SELECT 1 FROM conversation_overrides co WHERE co.conversation_id = c.id)
      `, [hydrated.userId, previousConversationId]);
    }
