  ...
  it('keeps counts and books stable, refuses ambiguous accounts, and preserves source identities', async () => {
    ...
    try {
      ...
      await client.query(`
        ...
        CREATE TABLE email_accounts (id uuid PRIMARY KEY, user_id uuid, provider_connection_id uuid, email_address text);
      `);
      ...
      await client.query("INSERT INTO email_accounts VALUES ($1,$2,$3,'same@example.test')", [accountA, owner, connection]);
      ...
      await client.query("INSERT INTO email_accounts VALUES ($1,$2,$3,'same@example.test')", [accountB, owner, connection]);
      ...
      await client.query("INSERT INTO email_accounts VALUES ($1,$2,$3,'same@example.test')", [unrelatedAccount, owner, randomUUID()]);
      ...
      expect((await list()).rows[0]).toMatchObject({ account_id: null, account_email: null });
      const davBook = randomUUID(); const source = randomUUID();
