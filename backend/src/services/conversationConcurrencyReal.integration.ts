...
describe('Conversation Engine v2 — real PostgreSQL concurrency', () => {
  it('25 concurrent merge A→B / B→A runs deterministically without cycles or cross-edges', async () => {
    ...
    assert.ok(failures.every(x => /cycle|alias|serialization|deadlock|different target|concurrent update/i.test((x as { error: { message: string } }).error.message)), `unexpected race errors: ${failures.map(x => (x as { error: { message: string } }).error.message).join('; ')}`);
