...
// Compute the thread_id for an incoming message using only strong RFC relations.
// Subject alone is intentionally not evidence: unrelated mail such as recurring
// notifications with a generic subject must remain separate conversations.
async function computeThreadId(accountId, messageId, inReplyTo, references) {
  ...
  // Keep a provisional RFC anchor; a later sync/rebuild can resolve it when the
  // referenced parent arrives. Never fall back to subject grouping here.
  return candidates[0];
}
