...
interface SenderFaviconCacheDouble {
  values: Map<string, string>;
  get: Mock<(key: string) => Promise<string | null>>;
  set: Mock<(key: string, value: string, options?: { EX?: number }) => Promise<unknown>>;
  del: Mock<(key: string) => Promise<unknown>>;
}

function cacheDouble(initial: Map<string, string> = new Map()): SenderFaviconCacheDouble {
  return {
    values: initial,
...
describe('getSenderFavicon', () => {
  it('fetches the exact fixed domain-only URL and caches a validated PNG for seven days', async () => {
    ...
    const fetchImpl = vi.fn<typeof fetch>(async () => response(png(64)));
  ...
  it.each([
    ...
    const fetchImpl = vi.fn<typeof fetch>(async () => response(png()));
  ...
  it('evicts an invalid cached PNG and refetches', async () => {
    ...
    const fetchImpl = vi.fn<typeof fetch>(async () => response(png(32)));
...
describe('getSenderFavicon parent walk-up', () => {
  ...
  it('resolves a not-found subdomain to its registrable parent, caching under both keys', async () => {
    const cache = cacheDouble();
    const fetchImpl = vi.fn<typeof fetch>(async target => (String(target).includes('/notion.so/') ? response(png(64)) : notFound()));
  ...
  it('walks on a provider 400 the same as a 404 (unresolvable mail-only subdomain)', async () => {
    ...
    const fetchImpl = vi.fn<typeof fetch>(async target =>
      (String(target).includes('/wanver.shop/') ? response(png(64)) : response('bad request', 400, 'text/html')));
  ...
  it('walks to a registrable domain but stops before a co.uk public suffix', async () => {
    const cache = cacheDouble();
    const fetchImpl = vi.fn<typeof fetch>(async target => (String(target).includes('/acme.co.uk/') ? response(png(64)) : notFound()));
  ...
  it('never queries a stoplisted public suffix while walking', async () => {
    const cache = cacheDouble();
    const fetchImpl = vi.fn<typeof fetch>(async () => notFound());
  ...
  it('caches an aggregate transient under the original key when a parent step is transient', async () => {
    const cache = cacheDouble();
    const fetchImpl = vi.fn<typeof fetch>(async target =>
      (String(target).includes('/notion.so/') ? response('', 503, 'text/plain') : notFound()));
  ...
  it('coalesces a subdomain walk and a direct parent request onto one parent fetch', async () => {
    ...
    expect(urls(fetchImpl).filter(target => String(target).includes('/notion.so/'))).toHaveLength(1);
  });
  ...
  it('caches an intermediate parent under its resolved image, not the miss the walk observed', async () => {
    const cache = cacheDouble();
    const fetchImpl = vi.fn<typeof fetch>(async target => (String(target).includes('/corp.com/') ? response(png(64)) : notFound()));
  ...
  it('gives a direct request racing the walk the resolved image, not the exact-fetch miss', async () => {
    ...
    expect(urls(fetchImpl).filter(target => String(target).includes('/b.corp.com/'))).toHaveLength(1);
  });
