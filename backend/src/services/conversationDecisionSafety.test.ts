...
describe('conversation v2 decision safety', () => {
  ...
  it('requires explicit smart-series enablement', () => {
    ...
    const strictDecision = strictSeriesDecision({ message: base, previous: base });
    if (strictDecision === null) throw new Error('Expected strict series decision');
    expect(strictDecision.kind).toBe('automated_reference_series');
  });
});
