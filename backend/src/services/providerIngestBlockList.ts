...
/**
 * Run the block list over the INBOX rows a provider sync just stored (MAIL-01).
 *
 * The IMAP sync has always done this for the messages it fetched; a native account stored its mail and applied
 * nothing, so a blocked sender's mail still arrived in the inbox. The rules engine itself is transport-neutral
 * now — it acts through `MailActionPort` — so the only provider-specific part is this call: pick the run's INBOX
 * rows and hand them to the same engine, with a port that speaks to Gmail or Microsoft Graph.
 *
 * Only the **block list** runs here. The rules engine's other actions (forward, archive, label, mark read) are
 * reached through the same port and can be enabled in the same place once they are exercised for a native account;
 * enabling them silently in one change would put every rule's side effect on an untested transport at once.
 *
 * A failure is logged, never thrown: an ingest side effect must not fail the synchronisation that stored the mail,
 * which is the rule the IMAP path already follows.
 */
export async function applyBlockListToIngestedRows(input: {
  userId: string;
  connectionId: string;
  ...
  providerName?: string;
}): Promise<{ considered: number; blocked: number }> {
  if (input.rowIds.length === 0) return { considered: 0, blocked: 0 };
  ...
  if (rows.rows.length === 0) return { considered: 0, blocked: 0 };
  ...
  try {
    const remaining = await applyBlockList(messages, {
      id: input.account.id,
      user_id: input.account.user_id,
      folder_mappings: (input.account.folder_mappings ?? null) as never,
    }, port);
    return { considered: messages.length, blocked: messages.length - remaining.length };
  } catch (caught) {
    console.warn(
      `Block list failed for ${input.providerName ?? 'provider'} account ${input.account.id}:`,
      caught instanceof Error ? caught.message : caught,
    );
    return { considered: messages.length, blocked: 0 };
  }
}
