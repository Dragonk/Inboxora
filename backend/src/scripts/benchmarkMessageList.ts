...
try {
  ...
  for (const [name, accountId, threaded] of cases) {
    ...
    report.cases[name] = { timingsMs: timings, medianMs: sorted[Math.floor(sorted.length / 2)], rows: Array.isArray(result.messages) ? result.messages.length : 0, total: Number(result.total ?? 0) };
