...
try {
  ...
  async function createConversation(accountId: string, subject: string, logicalCount: number, copyCount: number) {
  ...
  async function createLogical(accountId: string, conversationId: string, index: number, direction: string, subject: string) {
  ...
  async function createCopy(accountId: string, uid: number, folder: string, logicalId: string, conversationId: string, canonicalId: string, subject: string, bodyText: string, bodyHtml: string, direction: string, dayOffset: number) {
    ...
    await client.query(
      `INSERT INTO messages (account_id, uid, folder, message_id, subject, from_name, from_email, to_addresses, date, snippet, body_text, body_html, is_read, thread_id, logical_message_id, conversation_id, conversation_user_id, canonical_message_id, threading_reason, threading_confidence)
       VALUES ($1::uuid, $2::int, $3::text, $4::text, $5::text, $6::text, $7::text, $8::jsonb, NOW() - ($9::text || ' days')::interval, $5::text, $10::text, $11::text, true, $13::text, $12::uuid, $13::uuid, $14::uuid, $4::text, 'playwright-fixture', 1)`,
  ...
  // Gmail-only fixture: all copies share one native thread_id; folders still
  // provide the cross-folder physical-copy scenario for the real app.
