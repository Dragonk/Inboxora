...
if (!userId) {
  const result = await query('INSERT INTO users (username, password_hash, is_admin) VALUES ($1, $2, true) RETURNING id', [username, passwordHash]);
  userId = result.rows[0].id;
} else {
  await query('UPDATE users SET password_hash = $1, is_admin = true WHERE id = $2', [passwordHash, userId]);
}
...
for (const [name, email] of [['Gmail fixture', 'me@gmail.test'], ['Outlook fixture', 'me@outlook.test'], ['Fastmail fixture', 'me@fastmail.test']]) {
  const result = await query('INSERT INTO email_accounts (user_id, name, email_address, auth_user, enabled) VALUES ($1, $2, $3, $3, true) RETURNING id', [userId, name, email]);
