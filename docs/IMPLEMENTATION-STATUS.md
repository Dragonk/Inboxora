...
| P09 — Google Calendar/People + MS Graph calendar/contacts | **partial** | `29bf023e` (People), `71558193` + `c8ea8383` (Calendar with generated VTIMEZONE), `d4927e09` + `8aff1d1e` (UI), `a2973f94` (schedule); Microsoft contacts under P07 | Delivered: discovery with per-collection switches, read-only pulls for Google People, Google Calendar (VTIMEZONE included) and Microsoft contacts, and the refresh schedule. **Microsoft calendars are now imported** (P07d read path: discovery, a local read-only calendar per Graph calendar linked by its immutable id, a per-collection delta cursor with a rebuild that reconciles, structured `recurrence` → `RRULE`, generated `VTIMEZONE`, all-day and `RECURRENCE-ID` overrides, and the provider's `canEdit` recorded as the collection's source access). Missing: **all provider-side CRUD** — no calendar or contacts adapter issues anything but `GET` (verified: no `POST`/`PATCH`/`DELETE` in `googlePeople.ts`, `googleCalendar.ts`, `graphContacts.ts` or `graphCalendar.ts`'s write helpers are unwired) — plus provider-side mail search and the reply/forward dependencies. A read-only collection must stay read-only once writes exist. |
