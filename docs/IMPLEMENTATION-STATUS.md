...
Last re-measured on `dev` at `2224e80c`: backend typecheck, lint and **2438** unit tests
...
1. **Conversation persistence — half done, and the remainder is scoped, not open.** The identity half
   is delivered (`2224e80c`): `providerMetadataForMessage` recognises a Graph account and treats
   `thread_id` as a **strong** provider thread id. **What is missing is the caller** — the message sync
   does not persist through the conversation engine yet, so the mapping has no production path. The
   remainder was read rather than guessed:
   ...
   a. ~~widen the account parameter and treat a Graph account as its own provider~~ — **done**
      (`2224e80c`);
   b. ~~use `thread_id` as a strong provider thread id, not the Thread-Index path~~ — **done**
      (`2224e80c`);
