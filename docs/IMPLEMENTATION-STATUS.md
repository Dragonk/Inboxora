...
1. **The Google half of P09** — Calendar/People CRUD and the mail migration recommendation with *Ignore*
   + "do not show again" suppression stored **per user and per account** (`account_notice_preferences`
   already has that exact shape from `0101`; what is missing is the endpoints and the surface). In flight
   in this checkout as of this writing.
2. **`ruleForwarder.ts`** still reads and sends over IMAP/SMTP, so an automated inbox-rule forward on a
   native account would open IMAP; the fix is being written separately (read over the account's own
   transport, send through the seam).
3. **P14** — the final gate matrix on the frozen SHA, the `:dev` image pair from that exact SHA with its
