...
| P13 — hardening, E2E, release notes | **partial** | `065b3f86` (provider setup procedure), `6cccd470` + `6cf1a4bf` (imports), the DAV work under P11, the browser gate and the acceptance reports above; i18n kept at nine locales | Delivered: the end-to-end suite for the touched flows — **including the connector card, which had no browser coverage until this work gave it one** — the wiki pages, current UI copy, the unsupported-operations list and the W-list/matrix reports. Open: **release notes for a chosen version** (needs the version decision), the real-client DAV run, and the **installation-wide switch**: `PROVIDER_INTEGRATIONS_ENABLED=0` stops the four authorization flows and the readiness report from offering anything, read from one place, with unset meaning enabled. **It is not a guarantee that no outbound provider call happens** — the three sync routes and the scheduled refresh call the adapters directly, so a manual or scheduled sync for collections pulled earlier still reaches the provider. Closing that needs the same check in those four places, and it is the one thing about the switch a reader should not assume. The per-provider and per-method switches still say which parts a configured installation offers. |
