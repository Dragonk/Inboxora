...
**Frozen code SHA: `f0eba45ce481d47ea284ba76205b8c1b38007ca6`** — the revision that fixes the 4.0.4 → 4.1.0 upgrade path (0108/0114), adds
...
| Part | State | Evidence |
| --- | --- | --- |
| image build + registry verification | **done** | Workflow run [`35533356657`](https://github.com/Dragonk/Inboxora/actions/runs/35533356657) built from `source_sha=f0eba45ce481d47ea284ba76205b8c1b38007ca6` and pushed `:dev`. Both resolve to OCI image indexes carrying `linux/amd64` **and** `linux/arm64`: backend `sha256:8314505d7bd41d0486498342976516dad2e9ebea2b9c56733ff5b2de961acbb8`, frontend `sha256:911b65fd701e80ddbb7be04e6c306243cb3929da072c284b0afa09c97bc42137`. |
| runtime smoke | **RUN — passed** | The published pair was pulled from GHCR and started from a fresh volume: `/api/health` → `{"status":"ok"}`, `/api/version` → `{"version":"dev","sha":"f0eba45ce481d47ea284ba76205b8c1b38007ca6"}`, **117** migrations, first-user registration, a fresh login, `/api/auth/me`, the account list, the UI root and **0 restarts**. (`docker compose pull` hit a Docker Hub `429` on the third-party ntfy image, so the two Inboxora images were pulled explicitly and the stack started from the cached third-party ones; the upgrade smokes from a 4.0.4-shaped database and from an earlier-`:dev` database remain recorded below.) |
...
- **Backend** — typecheck clean, lint clean, **3021 unit tests passed** (230 skipped; 249 files).
- **Frontend** — typecheck clean, lint clean, **2866 tests passed** (0 failed), production build clean.
- **Database** — a database created empty for the purpose, the **whole 117-migration chain applied from
  zero** by the application's own runner, then **461 integration tests across 53 suites** (with the conversation regression suite in its own invocation) on PostgreSQL 16
