...
Last re-measured on `dev` at `44886e29`: backend typecheck, lint and **2434** unit tests
...
3. **Rules and the GTD/plugin abstractions** — checked, and the check found work rather than an
   ...
   **transition path calls `imapManager.removeMessageCopy`**, so a native account would build label
   copies and then fail to remove them. The second is not yet named to the user anywhere, and it is
   the next thing to look at here — before the folder setup is made to work, or GTD would start and
   then fail a layer deeper. Rules themselves were not found to touch the transport.
