...
the remaining rows are delivered (drafts, provider-side search, the Graph calendar adapter), which is the
list below.
...
| W06 | Microsoft: full mail over Graph plus that account's calendars and contacts | **FAIL** | Contacts are delivered; the mail **folder tree** (`705b13bc`), **message metadata with a per-folder delta** (`011b2251`), **read/star flag mutations** (`7076f4e1`), the **body with attachments** (`08f40ad1`), **delete** (`90bd9d02`), **move/archive** (`7ff67202`), **spam/ham** (`694141eb`), **snooze** (`ed3dbc18`), **bulk delete** (`30372761`), **mark all as read** (`24355f76`), **source headers** (`de85be4d`) and the **attachment ZIP** (`b9177c3f`) are delivered too. What is not: drafts, send, provider-side search and the reply/forward dependencies, and the **Graph calendar adapter** (P07b continues, P07d). Conversation persistence is delivered and is not the missing piece here. The row asks for full mail, so it stays FAIL rather than partial — a mailbox you can read but not file or answer from is not mail. |
