...
Last re-measured on `dev` at `cbe61df8`: backend typecheck, lint and **2435** unit tests
...
3. **Rules and the GTD/plugin abstractions** — checked, and the check found work rather than an
   ...
   **transition path called `imapManager.removeMessageCopy`**, so a native account would have built
   label copies and then failed to remove them — now provider-aware (`cbe61df8`), and fixed **before**
   the folder setup so GTD cannot start and then fail a layer deeper. What is left for GTD on a native
   account is the folder setup itself: `create` exists, so `ensureLabelFolders` can be given a provider
   path the same way the folder routes were. Rules themselves were not found to touch the transport.
