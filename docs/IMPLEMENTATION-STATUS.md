...
Last re-measured on `dev` at `ed3dbc18`: backend typecheck, lint and **2412** unit tests
(**116 skipped** across 14 files); frontend typecheck, lint, production build and **2685** tests;
**184** database integration tests across **18** suites on a fresh PostgreSQL 16 with the full
...
- 184 integration tests pass across eighteen suites: the provider authorization-flow table, Google and
...
| Rows | Verdict |
| --- | --- |
| RE01 | **Met.** The same functional schema is reached two ways and both were exercised: the state-reconstructing suite seeds a database, applies the chain and asserts the identifiers survive, and the database gate applies all 112 migrations to a fresh PostgreSQL 16 before running 184 tests. |
...
| W06 | Microsoft: full mail over Graph plus that account's calendars and contacts | **FAIL** | Contacts are delivered; the mail **folder tree** (`705b13bc`), **message metadata with a per-folder delta** (`011b2251`), **read/star flag mutations** (`7076f4e1`), the **body with attachments** (`08f40ad1`), **delete** (`90bd9d02`), **move/archive** (`7ff67202`), **spam/ham** (`694141eb`) and **snooze** (`ed3dbc18`) are delivered too. What is not: drafts, send, conversation persistence, search and the reply/forward dependencies, and the **Graph calendar adapter** (P07b continues, P07d). The row asks for full mail, so it stays FAIL rather than partial — a mailbox you can read but not file or answer from is not mail. |
...
- A Microsoft account's **folder tree, message list, body and attachments** are imported;
  **read/unread, star, move, archive, delete, spam/ham and snooze** work over Graph, where delete
