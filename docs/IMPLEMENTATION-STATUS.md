...
Last re-measured on `dev` at `8987e9f9`: backend typecheck, lint and **2431** unit tests
...
provider is folder `delete` and `empty` — and that is the whole list:

| Route | IMAP call it makes | What a Graph account gets |
| --- | --- | --- |
| `POST /folders/empty` | `emptyFolder` | **Refuses with `501`** (`2929f691`) and is the only row left. It has no direct Graph equivalent: it needs pagination over the folder's messages plus a decision about what emptying a folder the provider owns should mean — whether Inboxora removes each message permanently or moves them to deleted-items. `create`, `rename` and `delete` are implemented (`675494ad`, `8987e9f9`); a *nested* create still refuses, because Graph nests by parent id while the local model carries a path. |
