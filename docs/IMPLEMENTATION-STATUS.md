...
| A time-range filter must use the real recurrence projection, with `SQL OR recurring` only selecting candidates | **Was a defect; now fixed.** The query selected candidates and the handler returned all of them, so a series whose rule never lands in the window came back anyway — the chapter's warning exactly. Candidates are now filtered by `projectCalendarResource(row, start, end)`, which is the same projection the rest of the calendar code uses, and the existing calendar-query cases pass with it. **Its own case was written, failed on this suite's per-query database mock arrangement rather than on the fix, and was removed** — so the new branch is exercised by the existing cases and not yet pinned by one of its own, which is a coverage gap to close rather than a claim to make. |
