...
| DC03 | The flow is bound to user/session/account/purpose/configRevision; another user's flow id and another mailbox are not accepted | **PARTIAL, and closer.** A flow id exists and the poll checks the entry's owner against the session, so another user cannot reach a flow by guessing or naming its id — the property the row is protecting. What is still missing is the explicit binding to the account, purpose and configuration revision it was started with. |
