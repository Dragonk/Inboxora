...
Last re-measured on `dev` at `30372761`: backend typecheck, lint and **2416** unit tests
(**116 skipped** across 14 files); frontend typecheck, lint, production build and **2685** tests;
**186** database integration tests across **18** suites on a fresh PostgreSQL 16 with the full
...
- 186 integration tests pass across eighteen suites: the provider authorization-flow table, Google and
...
| Rows | Verdict |
| --- | --- |
| RE01 | **Met.** The same functional schema is reached two ways and both were exercised: the state-reconstructing suite seeds a database, applies the chain and asserts the identifiers survive, and the database gate applies all 112 migrations to a fresh PostgreSQL 16 before running 186 tests. |
...
`bulk-delete` was the first row of this table and is now wired (`30372761`); the rest are reachable
today and **not** wired to the provider:
