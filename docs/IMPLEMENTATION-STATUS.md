...
1. **Google Calendar/People CRUD** — mirror `services/providerCalendarWrites.ts` and
   `providerContactWrites.ts` with `googleCalendar.ts` / `googlePeople.ts` as the provider half, then
   flip `google_api.writeThrough` in `providers/registry.ts` **last** (the registry comment says why).
   `PATCH /integrations/collections/:id` already refuses with `WRITE_PATH_UNAVAILABLE` until that flip,
   so the interface cannot offer a Google write the server would refuse.
2. **The Google mail migration recommendation** with *Ignore* + "do not show again" suppression stored
   server-side **per user and per account** — the notice text exists (`acd1bf78`/`35451f02`), the
   suppression does not.
3. **P12** — the in-place Microsoft cutover (same `email_accounts.id`, no duplicate, Graph only
   afterwards).
4. **P14** — the final gate matrix on the final SHA, the `:dev` image pair from that exact SHA with its
   digests and architectures, then the runtime smoke (health, version, login, basic UI, account
   listing, no migration crash). Real-provider acceptance and DAVx⁵ stay **NOT RUN** unless credentials
   or clients are actually available.
