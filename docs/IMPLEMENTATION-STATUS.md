...
| P06 — send/draft ledger, attachment and MIME limits | **partial** | `379c8865` (one definition for the four size dimensions, and a domain code on the last of the three size guards), `b2e1c70e` (the `sendTransport` seam: one place that binds a send to a transport), `1962f519` (explicit, measured envelope), `3e9b0054` (the composed artefact carries no `Bcc` header), `0b93d799` (the message is composed once), `8013449e` (the Graph send adapter), `d5c66ee0` (attachments), `d38b53f5` (the staging draft), `b3018e56` (the Graph send), `7a8c9523` (the transport) and this work's seam wiring | **Delivered**: the four size dimensions in one definition (`services/sendLimits.ts`), the single transport seam, a measured explicit envelope (`to`+`cc`+`bcc`), a composed artefact proven free of the `Bcc` header, one composition instead of two, and a durable `send_idempotency` claim (`claim` / `uncertain` / `complete` / `release`) whose relation to `provider_operations` is written down — the HTTP replay belongs to the intent, the provider mutation to the journal, and no third mechanism is added. **Missing**: the MIME/content-type dimension and per-provider effective limits — a native Graph account is still refused above the installation's 25 MB attachment total even though Graph's own upload-session ceiling is 150 MB per file. **Delivered since the previous revision of this row**: the Graph branch wired into the seam (no `if (microsoft)` in the route), the small-attachment path, the upload session with resume/expiry/cancel, and the staging draft's upload-failure recovery, with the three transport outcomes mapped onto the intent lifecycle. Partly enforced today and worth naming so the package is not read as untouched: `send.ts` refuses more than 100 attachments and a total above 25 MB, `MAIL_MAX_MESSAGE_BYTES` counts the **composed** message and refuses with `413 MESSAGE_TOO_LARGE` before dispatch (tested), and the HTTP body limit is 35 MB with a route-aware message. |
