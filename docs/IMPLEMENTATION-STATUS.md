...
| AU04 | Bad or replayed state, nonce, issuer, audience and PKCE are rejected without changing the account | **PARTIAL, by reading.** `takeAuthorizationFlow` consumes the flow (a replay finds nothing), the comparison above refuses another session's flow, and the code for a changed configuration exists (`CONFIG_CHANGED`). The nonce/issuer/audience/PKCE parts are implemented in the identity and token paths but were not read in this pass, so they are unverified rather than claimed. |
