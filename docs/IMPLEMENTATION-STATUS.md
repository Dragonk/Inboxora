...
Last re-measured on `dev` at `27fbe525`: backend typecheck, lint and **2430** unit tests
...
| --- | --- | --- |
| `POST /folders/delete`, `/folders/empty` | `deleteFolder`, `emptyFolder` | **Refuse with `501`** (`2929f691`), and now the only two rows left. `create` and `rename` are implemented (`675494ad`); a *nested* create still refuses, because Graph nests by parent id while the local model carries a path. `delete` needs the provider call **plus** local cleanup, because discovery upserts folders but never removes one that vanished at the provider; `empty` has no direct Graph equivalent and needs pagination over the folder's messages plus a decision about what "empty" means for a folder the provider owns. |
