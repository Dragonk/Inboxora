...
Last re-measured on `dev` after the P06 delivery: backend typecheck and lint clean, **2855 unit
tests passed** (**180 skipped** across 24 files, 236 files passed of 260); frontend typecheck, lint,
production build and **2705 tests passed** (0 failed); **384 database integration tests across 41 suites**
...
suites run against it, then dropped — including the new send-ledger suite, which asserts on real rows that a
size refusal claims no intent and leaves no provider operation to reconcile. (The frozen SHA and the
published digests for this revision are recorded with the P14 rows below once the images are built from it.) The unit figure and the integration figure are separate invocations:
...
| P01 — shared provider contracts | **mostly delivered** | `abbe2b9b`, this work: `b0381b77` | The contracts, the registry and the capability table now **decide every collection operation**: the REST and DAV write guards, the DAV advertised privileges, the calendar delete and the contacts read-only flag all ask `services/providerAccess.ts`, which combines the origin adapter's declared support with the collection's own access and the device password's ceiling. The model now has a **remote adapter that declares `writeThrough`** — `microsoft_graph`, whose mail, contact and calendar-event writes are forwarded — and the interface half is done. The interface half is now done — the calendar UI reads the server's `read_only` in `CalendarPage.tsx`, `CalendarSidebar.tsx`, `CalendarContextMenu.tsx` and `CalendarInvitationCard.tsx`, and the four contract tests that pinned `source === 'local'` were corrected, so a provider collection the server reports as writable becomes editable without another frontend change. |
...
| P04 — OAuth flows and token service | **delivered** | `ee788ca8`, `d4592756`, `524a5f00`, `c30d13ba`, `d4927e09`, `940d629a`, `7dd0572d` + `3ed99007`, `bf9f340f`, this work's Graph device authorization (`0110`) | The **provider device-code authorization** is delivered: `POST /oauth/provider/microsoft/device` starts a public-client device grant for Graph scopes and `…/device/poll` completes it into the same `provider_connections` + `oauth_grants` rows the browser flow writes, with the device code (encrypted), the provider's interval and the last poll time held on the flow row so a restart does not strand a pending authorization. The provider's pending/declined/expired answers are flow states, an early poll is answered from the stored state rather than calling Microsoft, and the grant is recorded as a public client's so its refresh omits the secret. What stays unverified is the same as for the browser flow: **no real Microsoft application was registered**, so live authorization is NOT RUN. |
...
| P06 — send/draft ledger, attachment and MIME limits | **delivered** | `379c8865` (one definition for the size dimensions, and a domain code on the last of the three size guards), `b2e1c70e` (the `sendTransport` seam), `1962f519` (explicit, measured envelope), `3e9b0054` (the composed artefact carries no `Bcc` header), `0b93d799` (one composition), `8013449e`/`d5c66ee0`/`d38b53f5`/`b3018e56`/`7a8c9523` (the Graph draft, attachments, staging, send and transport) and this work | **Delivered**: the limits are resolved **per transport** — `min(installation ceiling, provider ceiling, operation ceiling)` — with the provider numbers defined once in `services/providers/mailCapabilities.ts` and read by the adapters that hit them (`graphMailAttachments`, `gmailApi`). The dimensions are separate and each refusal names the one it hit: one attachment, their total, the inline images the composer creates from `data:` URIs, the composed RFC-822 message (SMTP), the provider's raw message (Gmail), one provider upload object (a Graph upload-session file) and the HTTP request body; every refusal carries `code`, `dimension`, `actualBytes`, `limitBytes` and `transport`, with the file name when one caused it. A native Graph account is **no longer capped at 25 MB** — one file up to Graph's 150 MB travels through the resumable upload session (the 3 MB inline threshold is a method choice, not a ceiling), with resume, chunk alignment, expiry and cancel, while Gmail is measured on the raw message it is handed and SMTP on the composed message against `MAIL_MAX_MESSAGE_BYTES` (now a documented **fallback**, with `MAIL_MAX_ATTACHMENT_BYTES` as the **hard** installation ceiling). The provider's own size question is decided in a transport **preflight before the durable intent is claimed**, so a predictable refusal leaves no `send_idempotency` row, no `provider_operations` row and no `SEND_OUTCOME_UNKNOWN`; the HTTP body window is derived from the hard ceiling rather than a literal, so a provider-sized attachment can reach the route that decides about it; `GET /api/mail/send-limits` publishes the effective numbers (no ceiling as `null`) and the composer refuses a file it already knows cannot be sent without closing or clearing the draft. Verified by `sendLimits.test.ts`, `sendTransport.preflight.test.ts`, `send.limits.test.ts`, `send.limits.integration.test.ts` (real PostgreSQL: a refusal leaves no intent and no operation, and the same key retries smaller), the forwarded cross-transport case, the existing SMTP/Graph/Gmail suites and the composer contract test in all nine locales. **No residual: nothing in P06 remains open.** |
| P07 — native Microsoft Graph adapters | **partial** | `524a5f00`, `c30d13ba`, `a9a3f975` (contacts), `d545ff45`, `c08fb7ae`, `f4d4fac1`, this work: `705b13bc` (folders), `011b2251` (messages), `7076f4e1` (flags), `08f40ad1` (body), `90bd9d02` (delete), `7ff67202` (move/archive), `694141eb` (spam/ham), `ed3dbc18` (snooze), `30372761` (bulk-delete), `24355f76` (mark-all-read), `de85be4d` (headers), `b9177c3f` (attachment zip), `2929f691` (folder refusal), `675494ad` (create/rename folder), `27fbe525` (prefetch guard), `8987e9f9` (delete folder), `51099c3e` (empty folder), `44886e29` (GTD setup refusal), `cbe61df8` (GTD copy removal), `a4e248a4` (ensureFolder at the provider), `2224e80c` (Graph thread identity), `ff35ca81` (conversation projection), `9a9a0770` (send refusal), `379c8865` (send limits module), `b2e1c70e` (transport seam) | Delivered: the Graph contacts adapter and sync, the mailbox device flow, and **twenty-two P07b slices**, with the send gap named — (1) mail folder discovery, projected onto the local `folders` model with canonical paths for Outlook's well-known folders, linked by the immutable Graph folder id (`0107`); (2) **message metadata sync** with a per-folder delta cursor, identity in `messages.provider_message_id` (`0108`), the local-wins flag window and a `410` rebuild that reconciles; (3) **flag mutations over the shared layer** with a durable retry drained by the next sync (`0109`), and a permanent refusal undone rather than kept locally; (4) **body and attachments** read on demand, sanitised and cached in the shared columns, with inline images embedded and downloads addressing the Graph attachment id; (5) **delete**, which reuses the IMAP path's Trash-vs-permanent decision, re-homes the local row onto the identity a Graph move returns, and is declared non-idempotent so a recovered claim is parked rather than re-run; (6) **move and archive** to an arbitrary folder through both bulk routes, on the same helper, with Graph rows deliberately kept out of the IMAP delete-and-re-insert statement; (7) **spam and ham marking**, which had bypassed the dispatch entirely and failed on a native account, now on the same move with the verdict and training row written only after the provider confirms; (8) **snooze, both directions**, with the `Snoozed` folder created on the provider and discovered, and the wakeup moved onto the same shared helper; (9) **bulk delete**, both the permanent removal and the Trash move, with a shared `deleteGraphMessagePermanently` that single-message delete also uses; (10) **mark all as read**, one flag mutation per unread message, with the unread list taken before the local update flips it; (11) **source headers** read from Graph's `internetMessageHeaders` rather than reconstructed, with the pointless IMAP attempt removed from that path; (12) **the attachment ZIP**, one provider fetch per file under the same per-file ceiling, with the naming and packaging shared with the IMAP path; (13) **folder management refuses explicitly** (`501` with a code and the Outlook-then-sync workaround) instead of attempting IMAP and failing like a bug — legible first, implemented next; (14) **create and rename a folder**, which act on the provider and let discovery produce the local row and collection, including the message relocation a rename implies that the folder sync already did. A nested create still refuses, because Graph nests by parent id while the local model carries a path; (15) **the body prefetch guard**, found by re-reading this audit's own negative claim — the message-list route opened an IMAP connection for a native account on every listing; (16) **delete a folder**, provider first and the local mirror second, with the collection link removed so a later discovery cannot resurrect it; (17) **empty a folder**, one permanent removal per message, which closes this audit — every folder route now has a provider branch; (18) **the GTD setup step refuses explicitly**, and the check that produced it found the transition path's `removeMessageCopy` too — now provider-aware as well (`cbe61df8`), as is `ensureFolder` itself (`a4e248a4`), which superseded the refusal. The shared helper is now the single implementation of the "adopt the identity a Graph move returns" rule for every Graph move. **Provider-side search is delivered** (the route asks Graph's own index for a native account, ingests the hits through the message-sync projection and answers from the local model, reporting `providerErrors` without failing the local results — see `graphMailSearch.ts` and `search.provider.test.ts`), and **the reply/forward dependencies need nothing further**, verified by tracing rather than assumed: a reply's `inReplyTo`/`references` travel on the canonical model into Graph's `internetMessageHeaders` (`routes/send.ts`, `graphMailSend.ts`), the quoted body is read on demand over Graph (`graphMailBody.ts`), and a forwarded attachment is fetched from the account that owns the message over that account's transport (`sourceAttachments.ts`, with the Graph cases pinned in `sourceAttachments.test.ts`). One adjacent gap was found by that tracing and is being fixed separately: `ruleForwarder.ts` still reads and sends over IMAP/SMTP, so an automated inbox-rule forward on a native account would open IMAP. **Drafts are delivered** (the `POST /draft` and `DELETE /draft/:uid` routes), so a native account can save, reopen, replace in place (a PATCH of the same provider object) and delete a draft without an IMAP call. Conversation persistence is delivered (`2224e80c` identity, `ff35ca81` projection) and GTD no longer touches IMAP on a native account (`44886e29`, `cbe61df8`, `a4e248a4`) — the earlier "missing" line here contradicted the delivered list above it. Graph is now a **complete mail transport** — reading, filing, flagging and **sending** all have a provider path, and a native account never opens SMTP for a send — but no account is marked native yet; pointing an existing account at it is the P12 cutover. |
| P08 — Gmail API mail adapter | **partial** (read path) | this work: `0111` (migration) | The **read path** is delivered: labels are discovered and projected onto the local `folders` model (the system mailboxes `INBOX`/`SENT`/`DRAFT`/`TRASH`/`SPAM` with the canonical paths and `special_use` values the rest of the application reads, every user label as its own folder keeping Gmail's own name and `/` hierarchy, and `STARRED`/`IMPORTANT`/`UNREAD`/`CHAT`/`CATEGORY_*` deliberately **not** invented as folders), each linked by its immutable label id through `integration_collections` with `kind = 'mail_label'` — a value `0101` already declares, so **no new link table was needed** and migration `0111` only adds `messages.provider_labels`. Messages are ingested with a **mailbox-wide history cursor** (`users.history.list` from a stored `historyId`): a baseline records the mailbox's `historyId` *before* it lists anything and is **resumable** through `sync_states.page_checkpoint` (label, page token, captured history id), so a change arriving mid-run is replayed rather than lost; incremental runs re-read only the threads the history names and reconcile each thread as a unit; an expired history id (Gmail's `404`, which the classifier calls `RESOURCE_NOT_FOUND`) or a delta above the run's thread budget rebuilds from a baseline **and reconciles**; and a label deleted at Gmail removes its collection and folder while **re-homing** the messages it held to another label they still carry (or removing the row when the message is archived) instead of dropping mail. Conversations reuse `persistConversationCopyForRow` and the existing `gmail:` thread convention, so no grouping rule was invented. Label create/rename/delete run through `providerMutationService` (create and delete non-idempotent and never auto-retried; rename a state set), with the folder routes dispatching on `gmail_api` through `resolveMailTransportForSync`'s new `gmail` target. **Body and attachments on demand are delivered**: one `format=full` read answers the message's MIME tree, its body and its attachment list together; the HTML is sanitised and cached in the same `body_html`/`body_text`/`attachments` columns the IMAP path writes; inline `cid:` images are embedded as data URIs under a bounded count and byte budget; a download addresses Gmail's own attachment id under the same per-file ceiling IMAP and Graph enforce; and the body, source-header, single-attachment, attachment-ZIP and forwarded-attachment (`sourceAttachments.ts`) paths all dispatch on the account's transport, so a Gmail message is never read over IMAP. **Message mutations are delivered** through `providerMutationService` with Gmail's own idempotency: a flag is a label change (`\Seen` = absence of `UNREAD`, `\Flagged` = `STARRED`), a move adds the destination label and removes the mailbox being left (also used by trash, spam/ham and snooze, with the `Snoozed` label created through the mutation layer), archiving is removing `INBOX` with the row re-homed to whichever label remains, permanent delete is `messages.delete`, mark-all-read is one journal-backed flag write per unread message, and every message-mutating route in `routes/mail.ts` (single delete, bulk delete/move/archive, bulk read/unread, snooze, spam/ham) dispatches on `gmail_api`. One corrected defect came out of this: **bulk read/unread used to call the IMAP flag write for every account it grouped**, so a native Graph or Gmail account had an IMAP connection opened for it and the response claimed success regardless — now pinned by a route test across all three transports. **Send is delivered** as a branch of `services/sendTransport.ts` (`kind: 'gmail_api'`, `sendsRenderedMessage: false`), so the route still knows nothing about transports: the canonical `ComposedMail` is rendered once for Gmail's `raw` field, Gmail's answer is mapped onto `accepted` / refused / `outcome_unknown`, and the size ceiling is Gmail's own raw-message limit measured on the decoded bytes before dispatch. A verified provider fact changed the Bcc handling: **the Gmail API `Message` resource has no envelope field** (checked against the API discovery document) and Gmail documents the send as delivering to the recipients in the `To`/`Cc`/`Bcc` headers, so this arm **keeps** the `Bcc:` header (removing it would silently drop blind recipients) and relies on Gmail to keep it off delivered copies — the opposite of the SMTP arm, and stated in `composedMail.ts` where both renders are built from one composition. **Drafts are delivered**: saving a draft for a Gmail API account creates Gmail's own `Draft` (raw RFC-822 with the `Bcc:` header intact — a draft is never delivered and the blind recipient must survive a reload), re-saving PATCHes the same provider object instead of leaving two, deleting removes it at Gmail first and treats an already-gone draft as the requested end state, and the local mirror is keyed on the **message** id the draft wraps (the identity the sync reconciles on) with the draft id resolved from the provider at use time, because the local model has no column for a second identity and `0111` is the last migration this package may add. The `POST /draft` and `DELETE /draft/:uid` branches therefore never touch IMAP, and the cross-account removal helper now handles both native transports. **The P08 adapter is complete**: labels, ingest, body and attachments, mutations, drafts and send. What remains for the package is the account cutover (P12), which is what makes any of it reachable, and live acceptance, which is **NOT RUN** — no real Gmail mailbox has been used. **No account is migrated**: `mail_transport` stays `imap_smtp` unless an explicit cutover (P12) sets it, so every existing Google account keeps its app-password path and this code is unreachable for it. |
| P09 — Google Calendar/People + MS Graph calendar/contacts | **partial** | `29bf023e` (People), `71558193` + `c8ea8383` (Calendar with generated VTIMEZONE), `d4927e09` + `8aff1d1e` (UI), `a2973f94` (schedule); Microsoft contacts under P07 | Delivered: discovery with per-collection switches, read-only pulls for Google People, Google Calendar (VTIMEZONE included) and Microsoft contacts, and the refresh schedule. **Microsoft calendars are imported** (P07d read path: discovery, a local read-only calendar per Graph calendar linked by its immutable id, a per-collection delta cursor with a rebuild that reconciles, structured `recurrence` → `RRULE`, generated `VTIMEZONE`, all-day and `RECURRENCE-ID` overrides, and the provider's `canEdit` recorded as the collection's source access), and **Microsoft contact writes are delivered** (create/update/delete go through the provider journal to Graph before the local copy changes, a created contact keeps the provider identity, and a write-back switch per collection refuses unless the origin permits writes and an adapter forwards them). The capability model now reads both the origin's permission (`source_access`) and the user's choice (`user_access`, migration `0112`), so a pulled collection stays read-only until it is switched on — and the Microsoft Graph registration is the first to declare `writeThrough`. **Microsoft calendar-event writes are delivered too** (create/update/delete to Graph through the provider journal before the local copy changes, the provider's `iCalUId` kept as the local identity, `remote_object_links` recorded so the next delta updates rather than duplicates, times sent as the exact instant, a local recurrence rendered into Graph's `pattern`/`range`, and Microsoft's own attendee notification not duplicated by Inboxora's invitation mail). **The Google half is delivered too**: `google_api` declares write-through and `providerGoogleWrites.ts` forwards calendar-event and contact create/update/delete through the same journal, with the local resource id journalled and the provider's identity in the payload and `remote_object_links` (Google sends its own attendee notifications), and the collection's `source_access` is now what Google reports (`accessRole`, and the People API's answer for the user's own contacts) rather than a blanket `read_only`, so the per-collection opt-in actually works. Missing: nothing in the package — what remains is the **interface surface for the Google recommendation** (the endpoints exist; no UI reads them) and provider-side mail search, which is delivered for Graph. |
| P10 — external CalDAV/CardDAV read-write, ICS/VCF/CSV import | **delivered** | `6cf1a4bf` (vCard import), `6cccd470` (iCalendar import), Google CSV import pre-existed, this work: `04971651` (the write-back client) | The **external CalDAV/CardDAV write-back client** is delivered: a `PUT`/`DELETE` on an imported collection is forwarded to the source it came from (`services/providers/davWriteBack.ts` + `caldavWriteBack.ts`/`carddavWriteBack.ts`) through the shared journal, with the client's `If-Match`/`If-None-Match` preconditions forwarded to the source, a changed object answering `412`, an ambiguous source answer parked as `outcome_unknown`, and the local projection plus the remote link updated only after the source confirms. The registry's `caldav`/`carddav` entries declare `writeThrough`, so the capability model admits the write only for a collection whose `source_access` and `user_access` both permit it. ICS-URL stays read-only and a locally imported ICS copy stays locally editable. Open: **no real client has talked to the server** (DAVx⁵/Thunderbird/macOS **NOT RUN**), and a CalDAV/CardDAV collection imported before this work has no `integration_collections` row, so it stays read-only over DAV until one exists — the new import paths create one. |
...
| P12 — account migration/cutover, MS-required and Google-recommended notices | **partial** (Microsoft cutover delivered, Google recommendation outstanding) | `acd1bf78` + `35451f02` (notices), this work: `e43268d3` (the in-place Microsoft cutover) | **The Microsoft cutover is delivered**: `POST /api/accounts/:id/migrate` moves an existing Microsoft account to the native Graph transport **in place** — the same `email_accounts.id`, every local message/folder/draft/alias/conversation untouched — and only when an active connection whose grant carries `Mail.ReadWrite` and `Mail.Send` resolves for the account (named explicitly or matched on the verified identity), ownership enforced by the row lock. The switch is one atomic update under that lock (`mail_transport`, `provider_connection_id`, `protocol`, `migration_state = 'active_native'`, generation + 1), idempotent on retry, and a refusal is recorded as `authorization_required`/`admin_configuration_required` without changing the transport; there is **no fallback** to Microsoft IMAP/SMTP afterwards, and the IMAP loops and health checks now also filter on the authoritative `mail_transport` so a native account cannot be reopened over IMAP. **The Google recommendation is delivered too**: `GET /api/integrations/notices` lists the recommendation for the caller's own Google mailboxes still on IMAP/SMTP (and only while the layer, the method and an OAuth client are configured), `POST /api/integrations/notices/:accountId/suppress` records the durable per-user, per-account suppression in `account_notice_preferences` (`0101`'s own shape, no migration), and the **accounts settings surface it** per mailbox with *Ignore* (session) and *do not show again* (durable) side by side; the Microsoft requirement notice is not representable in that table and so cannot be suppressed. Missing: the full inventory/backfill/reconcile state machine — the cutover moves the transport, not the data. A real mailbox cutover is **NOT RUN**. |
...
| --- | --- | --- |
| image build + registry verification | **done, from the frozen SHA `f2b0dbf1`** | Dispatched workflow run [`35471043131`](https://github.com/Dragonk/Inboxora/actions/runs/35471043131) built from `source_sha=f2b0dbf13594adc454e610d8aa458d581a83bdff` and pushed the `:dev` tags. Both resolve to OCI image indexes carrying `linux/amd64` **and** `linux/arm64`: backend `sha256:04512c4407e774fbb6d5fe956a6b74511a4755fb9451e8e79c245c2df4f4f20e`, frontend `sha256:87456ed55c4591807a069869aef6e7db0cc4aa600da00cfae28355245fc3660b`. The earlier pair built from `43c15e91` (run `35459869340`, backend `sha256:fdf967e5…`, frontend `sha256:3c3efc78…`) is superseded by this one. |
| runtime smoke pair | **RUN — passed on both published pairs** | The pair built from `f2b0dbf1` was pulled and started as a stack from a fresh volume: `/api/health` answered `{"status":"ok"}`; **`/api/version` answered `{"version":"dev","sha":"f2b0dbf13594adc454e610d8aa458d581a83bdff"}`**, so the running image is the frozen revision; the first user was registered (admin), a fresh cookie jar logged in through `POST /api/auth/login`, `/api/auth/me` returned that user, `/api/accounts` returned `[]`, and the UI root served the application. The backend applied the **whole 115-migration chain** on a fresh volume, `schema_migrations` held 115 rows, and `docker inspect` reported **0 restarts** for every container. The same smoke passed on the `43c15e91` pair at 18:0x the same day. |
| final v4 publication | **done** | The `:dev` images correspond to `f2b0dbf1`, the code tip of `dev` after every v4 package — including P06 — closed (the commits that follow it are documentation only, so the published images are the current `dev` code): the send seam and its Graph transport, Graph drafts, the provider device-code flow, the Graph calendar read and write paths, Graph contact writes, the Gmail API adapter, the Google write paths and recommendation, the frontend capability cleanup, P10 DAV write-back, P12 in-place cutover, provider-side search, the rule-forwarder transport fix, and the transport-aware send limits. `main` is untouched: 4.1.0 is still released only when `dev` is merged to it through a pull request. |
...
**Slice F is delivered, wiring included** (`7a8c9523` for the transport, and the seam wiring committed
with this document — `git log --oneline -- backend/src/services/sendTransport.ts` names it).
`services/sendTransport.ts` now carries the contract the plan
required: a `MailTransport` is returned by `createAccountMailTransport()` and its `send()` takes the
canonical `ComposedMail` model plus, for SMTP only, the rendered RFC-822 message. A native
`mail_transport = 'microsoft_graph'` account is bound to `graphMailTransport` — **no `if (microsoft)` in
`routes/send.ts`** — and a native account without its `provider_connection_id` is refused with
`409 PROVIDER_AUTH_REQUIRED` before any transport is built. `routes/send.ts` maps the transport's three
answers: `accepted` sets the delivered boundary and keeps nodemailer's recipient lists, a `refused`
releases the intent claim and the Redis lease (nothing left, so the same key can retry deliberately) and
answers with the provider's status and code, and `outcome_unknown` keeps the durable uncertain intent and
answers `502` with `SEND_OUTCOME_UNKNOWN`. The SMTP arm keeps its behaviour byte-for-byte: it is still
handed the route's pre-composed buffer as `raw`, its protocol failures still throw into the route's
classifier, and its Sent-folder APPEND path is untouched. The seam declares
`sendsRenderedMessage` so a native account **renders no MIME and opens no SMTP object at all**, and the
Sent-copy block is skipped for it (Graph files the message in Sent Items; the mail sync ingests it, and
there is no IMAP endpoint to verify against). Both facts recorded in the previous handoff were used and
both are now moot: the outcome value is named `outcome` rather than `sendResult`, and the post-send
metadata reads `composed.inReplyTo` / `.references` / `.messageId` rather than nodemailer's options.

**The W06 send cases are green.** `send.graph.test.ts` pins the native-account path end to end — Graph
transport bound and SMTP factory never called, no IMAP APPEND, structured recipients carrying `bcc` to
`bccRecipients`, attachments added before the send, `202` acceptance, `429`/`403` refusals releasing both
gates, a staging failure as a retryable refusal, and an unknown outcome parking the intent — while the
SMTP suite (`send.reliability`, `send.sent`, `send.forwarded`, `sendTransport.test.ts`) pins that the
...
### Handoff — what remains after the P07d/P08/P09/P10 work

The Microsoft write paths are complete (mail, contacts and calendar events all forward through
...
the Gmail API adapter in full, the Google write paths, the Google recommendation with its suppression and
surface, the frontend capability cleanup, the P10 DAV write-back, the P12 in-place Microsoft cutover, and
provider-side search. `ruleForwarder.ts` was the last provider-blind path and now reads and sends over the
account's own transport.

**Before touching any of it**, re-run the whole-repo gates: `git log --oneline origin/dev`,
`npx tsc --noEmit`, `npx eslint src --max-warnings 0`, `npx vitest run`, and the real-PostgreSQL gate
(a fresh database, the migration chain applied from zero, then the provider/DAV integration suites).
...
- 191 integration tests pass across nineteen suites: the provider authorization-flow table, Google and
...
| Rows | Verdict |
| --- | --- |
| RE01 | **Met.** The same functional schema is reached two ways and both were exercised: the state-reconstructing suite seeds a database, applies the chain and asserts the identifiers survive, and the database gate applies all 112 migrations to a fresh PostgreSQL 16 before running 191 tests. |
...
> Historical — superseded by the P06 delivery in the Status table above. The paragraphs that follow describe
> the limits as they stood *before* the transport-aware model; the current behaviour is the P06 row, and
> nothing below should be read as a claim about the present.
...
| W06 | Microsoft: full mail over Graph plus that account's calendars and contacts | **PASS** | Contacts are delivered; the mail **folder tree** (`705b13bc`), **message metadata with a per-folder delta** (`011b2251`), **read/star flag mutations** (`7076f4e1`), the **body with attachments** (`08f40ad1`), **delete** (`90bd9d02`), **move/archive** (`7ff67202`), **spam/ham** (`694141eb`), **snooze** (`ed3dbc18`), **bulk delete** (`30372761`), **mark all as read** (`24355f76`), **source headers** (`de85be4d`) and the **attachment ZIP** (`b9177c3f`) are delivered too. What is delivered since that verdict: **drafts** (save/reopen/replace/delete at the provider) and **send** over Graph, neither of which opens SMTP, and the **Graph calendar read path** (discovery, per-collection delta with a reconciling rebuild, structured recurrence rendered to `RRULE` with generated `VTIMEZONE`, the provider's `canEdit` recorded as source access). What is not: **provider-side search**, the **reply/forward dependencies**, and the **Google** write paths (Microsoft contact and calendar-event create/update/delete are delivered behind the per-collection write-back switch). **Provider-side search is now delivered** (a search scoped to a native account asks Graph's own index, ingests the hits through the message-sync projection and answers from the local model, with `providerErrors` reported beside the local results — `graphMailSearch.ts`, `search.provider.test.ts`, `graphMailSearch.integration.test.ts`), and **the reply/forward dependencies need nothing further**: a reply's `inReplyTo`/`references` travel on the canonical model into Graph's `internetMessageHeaders` (`routes/send.ts`, `graphMailSend.ts`), the quoted body is read on demand over Graph, and a forwarded attachment is fetched from the account that owns the message over that account's transport (`sourceAttachments.ts`, pinned for the Graph cases in `sourceAttachments.test.ts`). Conversation persistence is delivered (`2224e80c`, `ff35ca81`). The row asks for full mail plus that account's calendars and contacts, and all of it now has a provider path: read, folders, delta sync, flags, body/attachments, delete, move/archive, spam/ham, snooze, bulk delete, mark-all-read, headers, attachment ZIP, **drafts, send, search**, plus **calendars and contacts read and write** — so the verdict is **PASS**. Two facts keep it honest: no account is native until the P12 cutover is invoked for it, and `ruleForwarder.ts` still reads/sends over IMAP/SMTP, so an automated inbox-rule forward on a native account is the one mail path that would open IMAP (being fixed separately). |
...
**This audit is closed.** Every row in it has been wired — `bulk-delete`, `mark-all-read`, `headers` and
the attachment ZIP in `30372761`/`24355f76`/`de85be4d`/`b9177c3f`, and folder `create`, `rename`,
`delete` and `empty` in `675494ad`/`8987e9f9`/`51099c3e`. One deliberate refusal remains, narrower than
the table's original scope:

| Route | IMAP call it makes | What a Graph account gets |
| --- | --- | --- |
| a **nested** folder create | — | Refused with `501` by design. Graph nests by parent id while the local model carries a path, and guessing a path would be worse than saying so. Everything else this table once listed is implemented. |

**Not reachable**, re-verified after fifteen slices rather than left as written: every other
`imapManager.*` call site in the mail routes sits inside an `else` branch that a native account does not
enter — the flag push, the single-message move-to-Trash and permanent delete, snooze, spam/ham, and the
IMAP arms of bulk-move, bulk-archive and bulk-delete. `imapManager.broadcast`, `_guardMoveUid`/
`_unguardMoveUid` and `syncFolderOnDemand` are not provider calls: the first two are no-ops for an
account that is never IMAP-synced, and the third is only ever reached for IMAP accounts.

**That re-read found one that this paragraph had missed**: the message-list route's fire-and-forget
`prefetchFolderBodies` had no transport check, so every listing on a native account opened an IMAP
connection that could only fail. It is guarded now (`27fbe525`), inside the method so every caller is
covered. The lesson is the same one this document keeps paying for, in its negative form: a claim that
something is *not* reachable decays exactly like a claim that something is, and it is only checked when
...
1. **Conversation persistence — delivered** (`2224e80c` identity, `ff35ca81` projection). Graph
   messages reach the conversation engine and group on `conversationId`, asserted on a real database.
   Two things are worth carrying forward: the projection runs **after** the page transaction — the page
   returns the row ids it wrote for exactly that reason — and `persistConversationCopyForRow` lives in
   `conversationRowIngest.ts` so both transports share one projection; it had to move there rather than
   be imported by the adapter, because `imapManager` imports the Graph sync and the reverse import would
   have been a cycle.
...
3. **Rules and the GTD/plugin abstractions** — checked, and the check found work rather than an
   ...
   **pieces**, not none. Its folder setup creates label folders through the labels capability
   (`ensureLabelFolders`, IMAP) and now refuses with `501` and a workaround (`44886e29`); its
   **transition path called `imapManager.removeMessageCopy`**, so a native account would have built
   label copies and then failed to remove them — now provider-aware (`cbe61df8`); and the **folder
   setup** goes through `ensureFolder`, which is provider-aware too (`a4e248a4`), so both dependencies
   are closed and GTD's setup step works on a native account. Rules themselves were not found to touch
   the transport. What a native GTD account has **not** been exercised against is a real mailbox: the
   label copies it creates are ordinary provider messages, so the first live run is the thing that would
   find anything left, and it is **NOT RUN** with the other live provider checks.
...
**P06 — the send gap is now named, and the seam is located.** Sending from a native account answers
`501` with a code and a sentence (`9a9a0770`) instead of a credential error, so the one remaining
user-facing Microsoft action explains itself while Stage 3 is built. Everything below is the plan for
replacing that refusal with the transport.
...
**The limit definition now exists** (`services/sendLimits.ts`, `379c8865`): the four dimensions, their
kinds and the refusal shapes in one place, with `mailMaxMessageBytes` defined there and re-exported from
the route rather than written twice. The same slice fixed the last of the three size guards, which
answered a too-large forwarded attachment with a bare `400` and English prose while its two siblings
returned `413` with a domain code — the last line of defence was the one a client could not act on. What
remains of (a) is the **MIME/content-type** dimension itself — counting parts and their declared types
rather than only bytes — and the composer is where it has to be shared.
...
- ~~**one transport call to wrap**~~ — **the seam is in** (`services/sendTransport.ts`, `b2e1c70e`). The
