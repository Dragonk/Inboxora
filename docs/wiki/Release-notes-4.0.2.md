...
- Deploy database migrations `0085_calendar_invitation_outbox_claim.sql` through `0090_calendar_cancellation_outbox_reference.sql` in sequence before running the updated workers or mail-send route. Migration `0090_calendar_cancellation_outbox_reference.sql` must be applied before rollout so cancellation-status retries can remain bound to their original outbox row. Do not bulk-reset existing `uncertain` rows, because an earlier SMTP attempt may have been accepted.
