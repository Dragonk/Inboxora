...
| Attachment too large | The limit that applies is the **sending transport's**, and the refusal names the dimension it hit — one attachment (`ATTACHMENT_TOO_LARGE`), their total (`ATTACHMENTS_TOO_LARGE`), the inline images (`INLINE_IMAGES_TOO_LARGE`), the composed message on SMTP (`MESSAGE_TOO_LARGE`), Gmail's raw message (`PROVIDER_MESSAGE_TOO_LARGE`), a Microsoft Graph upload-session file (`PROVIDER_UPLOAD_TOO_LARGE`) or the request body (`REQUEST_TOO_LARGE`) — with the real byte count and the limit. The composer refuses a file it already knows cannot be sent before uploading it, and the server repeats every check. A Microsoft Graph account carries a single file up to its own upload-session ceiling; an SMTP account is bounded by `MAIL_MAX_MESSAGE_BYTES` and the hard `MAIL_MAX_ATTACHMENT_BYTES`. A message the installation accepts can still be refused upstream, because no provider's own limit is raised here. |
...
| No Google OAuth after an upgrade | The IMAP/SMTP method still works and needs no configuration. The provider card explains what an administrator has to register; once it is, the accounts settings recommend the API for a Gmail mailbox, with *Ignore* and *do not show again*. |
| A Microsoft migration is refused | The connection's grant must carry `Mail.ReadWrite` and `Mail.Send`, the application must be configured, and a named connection must belong to that mailbox. The account reports which of these it is and keeps working over IMAP/SMTP until it is fixed. |
| A DAV write-back is refused | Write-back is enabled **per collection**, the collection's source must allow writes (a calendar shared as read-only cannot be made writable), and an ICS subscription has no write channel at all. An external collection gets its write-back link on the next sync of its source. |
