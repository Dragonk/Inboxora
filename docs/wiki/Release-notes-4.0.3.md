...
- Defaults are safe: master switch on, per-account `antispam_enabled` off (opt-in via `PUT /api/accounts/:id` and the account form, alongside `trusted_authserv_id`). Migrations `0095`–`0097` must be applied in order before rollout.
- Frontend: `SpamBadge` verdict chip + explain dialog (mounted in `MessageDetailContent`, now fed by `spam_verdict`/`spam_score_ml` projections in the list, thread, message and resolve-message queries), `SpamSettings` status/master-switch/retrain panel mounted as Settings → Rules → Antispam (with search index entry), per-account antispam toggle in the account form, locale keys in all 9 locales.
...
- Backend: `tsc --noEmit` clean, `eslint src --max-warnings 0` clean, migration integrity suite clean (0094–0097). Focused regression suites cover physical-copy identity, explicit IDLE (attempted vs entered + single-flight + reject path), STATUS no-self-cancel, SEARCH-ALL-diff repair with maintenance-table marker (retry, per-folder verification), the single-MOVE auto-move (concurrency + reject), INBOX-only eligibility with pre-move revalidation, spam field projections, distinct-message maturity (duplicate-feedback, per-class minimum, legacy rows), threshold validation, and the per-user lock release — plus the full spam stack (tokenizer/rules/Bayes/store/pipeline/scheduler plus HTTP route tests for atomic training, per-account enable and explain-from-stored).
