...
- `calendarAgenda.test.ts`: complete date ranges, month transitions and multi-day ordering.
- `v3-interface.spec.ts`: agendas, visibility, imported details, rich-contact round trips, composer recipients, failed-save retry, scale and preferences, panel geometry, and visual references.
...
- `real-app.spec.ts`: real login/conversation rendering, rich contact edits and calendar creation/reload/deletion with PostgreSQL and Redis. Test-owned contacts and events are cleaned up.
...
`frontend/e2e/docs-screenshots.spec.ts` (see [Development](Development.md)).
