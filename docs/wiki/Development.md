...
- No global TypeScript install: `typescript` is a dev dependency of both projects.
...
| Path | Contents |
| --- | --- |
| `backend/` | Express API, IMAP/SMTP sync, conversation engine, CalDAV/CardDAV servers, SQL migrations — TypeScript, built with `tsc` into `dist/`. |
| `frontend/` | React application in TypeScript (`.ts`/`.tsx`), shared UI primitives, Playwright specs and visual references. |
...
Both projects are written in TypeScript and both must type-check cleanly before a change is pushed:
...
```bash
npx playwright test e2e/v3-interface.spec.ts --update-snapshots
```

A separate live-backend spec runs with a provisioned PostgreSQL and Redis:

```bash
PLAYWRIGHT_REAL_APP=1 PLAYWRIGHT_WEB_SERVER_COMMAND="..." npx playwright test e2e/real-app.spec.ts
```
...
`frontend/e2e/docs-screenshots.spec.ts` plus its demo dataset `frontend/e2e/docs-demo.ts`; it is
...
- Shared controls live in `ui.tsx` / `ui.css`; Calendar and Contacts have scoped presentation
  styles; email HTML keeps its own isolated renderer.
- `v3-interface.spec.ts` checks both behaviour and image references at desktop, tablet and mobile
