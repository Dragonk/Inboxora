...
- **Settings account views follow one responsive visual hierarchy.** Mail, Calendar and Contacts settings now lead with compact account/source cards, readable service status and an explicit management path; Calendar account details remain scoped to the selected source. The settings panels retain their existing safe operations and work with the active application theme on compact screens. Integration cards stay vertically stacked and collapsed by default, keeping global OAuth setup distinct from adding a user's account. No migration or new provider permissions are required.
