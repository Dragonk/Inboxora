...
- **Provider contact books include their account context.** The address-book read model now returns a provider collection’s connection, provider and mailbox identity. This makes identical book names from different accounts distinguishable and is the safe basis for per-account sync controls.
