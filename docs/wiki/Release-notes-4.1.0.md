...
- **Disabled providers stop background calls.** Scheduler and push-hint refresh now consult the provider/API switches before dispatching any adapter, so disabling an integration also stops syncs for connections authorized earlier. This does not change manual sync-route enforcement, which remains a separate follow-up. Unit tests cover disabled provider/API policy before scheduler or push-hint adapter dispatch.
