...
- **Provider account cards stay compact.** Google and Microsoft service controls, reconnect/retry and diagnostics are available after opening the specific account for editing rather than expanding every row in the Accounts list. Native account validation no longer asks for hidden IMAP fields.
