...
- **v2/v3 account and resource presentation correction.** Settings now share compact account cards with metadata footers and a list-to-detail flow. Contact books and calendar collections remain distinct from their connection accounts. Integration configuration keeps vertically stacked disclosures and single-column credential fields; existing OAuth transports and secret handling remain in place. Mail editing retains one form across tabs and coordinates its Save/Cancel boundary with staged service changes. System group labels are translated semantically, without renaming user-created books or calendars.

- **Selection and color behavior.** Group selection is one atomic array update, not repeated toggles over a stale snapshot. Contact visibility filters support an explicit empty selection and per-user persisted display/collapse preferences. A viewer can hide a remote/read-only book without being granted write-back or import access. Calendar colors distinguish the source color, a personal override and the resulting display color; preview/cancel/reset and the event reads use that distinction consistently, including the contact-dates calendar. Apply the existing `0138_calendar_color_overrides.sql` after `0137_calendar_collection_projection_receipts.sql` before running the corrected backend; no new migration is added here. Provider-native calendar operations retain their authorization and uncertain-operation safeguards. Creation/deletion of external DAV collections, Microsoft contact folders and Google groups is not added by this GUI correction. Validation supplied with the source bundle covers isolated components and pure helpers; whole-application typecheck, lint, i18n, tests, builds, migration checks and visual comparisons remain integration gates, not claimed completed by those isolated tests.

- **Prepared settings navigation is now reflected in the application.** Mail account editing has provider-specific General, Services or Servers, and Diagnostics sub-tabs; Calendar and Contacts separate accounts, collections and import tools; Integrations shares the same accessible provider/application tab treatment. These are presentation and navigation changes only: they add no migration, configuration requirement or provider permission, and retain the existing synchronization and remote-deletion safeguards.

- **Settings account views follow one responsive visual hierarchy.** Mail, Calendar and Contacts settings now lead with compact account/source cards, readable service status and an explicit management path; the mail overview has one prominent management action and secondary actions in an accessible compact menu. Calendar account details remain scoped to the selected source. A successful manual Calendar or Contacts run refreshes its account status immediately. Diagnostics exclude failures from retired, disabled, unlinked and tombstoned provider collections, but retain an error from any still-active collection, so a healthy account is not marked failed while a real live failure stays actionable. The settings panels retain their existing safe operations and work with the active application theme on compact screens. Integration cards stay vertically stacked and collapsed by default, keeping global OAuth setup distinct from adding a user's account. No migration or new provider permissions are required.

- **Google Calendar API remediation.** A disabled-Google-API error is a Google Cloud project configuration error, not a mailbox or OAuth identity error. Enable **Google Calendar API** in the same project as the configured Inboxora OAuth client, wait for Google's configuration propagation, then use the account card’s Calendar refresh action. The card now shows the Google setup link and an account-scoped retry button for Calendar (and independently for Contacts when affected); a generic page refresh or reconnect is not presented as a fix for an API that remains disabled. Once the same collection finishes cleanly, its historical failure timestamp is cleared. Diagnostics also filter legacy collection rows that recorded a failure before a newer success, so Calendar and Contacts do not report a recovered error as current; a failure newer than its own collection's last success remains visible.

- **Account-native calendar lifecycle.** Apply `0136_calendar_collection_lifecycle.sql` after `0135_contact_provider_email_uniqueness.sql`, then apply `0137_calendar_collection_projection_receipts.sql`, before rolling out this backend. Canonical Calendar Settings can create a name-only Google/Microsoft calendar for the selected account and request deletion only through an explicit confirmation; the server resolves the provider, connection and remote identity, then rechecks default/shared/ownership protection before writing. A confirmed create starts with event write-back disabled. Every action journals a canonical account/connection intent and returns confirmed only after its local projection receipt completes; a pending response can be checked again with the same key, without a second remote mutation. Confirmed deletion records a tombstone before removing its local calendar/events, so stale discovery or an in-flight sync cannot recreate it. Tombstones outlive account/journal cleanup while their provider connection remains. Non-idempotent unknown or legacy pending attempts remain parked. A retryable result records generation-bound proof that the provider did not apply it, honours the later of configured backoff and provider `Retry-After`, and consumes that proof before exactly one later re-dispatch; a crash after the new dispatch returns to the unknown state rather than repeating the remote call. Verification includes mocked provider/UI flows and PostgreSQL fence, journal, projection-receipt and upgrade checks; no real provider writes were performed.

- **Calendar lifecycle groundwork (not yet a UI action).** Account calendar diagnostics expose separate `calendarManagement` authorization: Google event scopes alone do not permit collection management, while `calendar.calendars` or full `calendar` and Graph `Calendars.ReadWrite` do. Existing event-write flags and ordinary consent requests are unchanged; the Google OAuth start route accepts explicit `manageCalendars=1` only with `purpose=calendar_enable`, a selected owned account and non-read-only access. This adds `calendar.calendars` to the flow's stored requested scopes; partial provider grants retain only their actual current capabilities. Internal create/delete helpers protect default/primary and shared calendars using fresh provider metadata, reject ambiguous identity, and require a confirmed delete response. They are wired only through account-scoped lifecycle routes and canonical Settings after durable replay, local projection recovery and deletion fencing; local generic calendar routes remain unable to simulate provider collection deletion. The authorization evaluator and low-level HTTP helpers themselves require no migration; the durable fencing layer below does. No ordinary reconnect silently requests calendar-management consent. Verification: 61 mocked native lifecycle tests, 13 cancellation regressions, the complete backend suite (3,330 passed; environment-gated integrations skipped there), and the expanded CI PostgreSQL selection (602 passed) succeeded; strict typecheck, lint and backend production build passed. Account diagnostics now run explicitly in the PostgreSQL CI selection. No real provider writes were performed.
- **Cancellation follows provider requests.** API clients compose caller signals with their own deadline rather than overwriting cancellation. Token refresh may still finish for its other consumers, but cannot dispatch a cancelled write or an extra 401 retry for this caller. This does not establish that an already-dispatched external write was rolled back; such outcomes must remain uncertain in the mutation journal.

- **Contact-book actions stay on their account.** Management groups use connection/account IDs, not mailbox labels. The address-book list returns an account only when its ownership is unambiguous and belongs to the same provider connection; duplicate mailbox links no longer multiply books or counts. Unresolved legacy ownership disables per-book sync instead of invoking a provider-wide sync. Partial or failed account results remain explicit, and new contacts keep their captured writable destination through save-time capability revalidation. No additional migration is required for this presentation change.

- **Graph contacts upgrade repair.** Apply `0135_contact_provider_email_uniqueness.sql` after `0134_contact_local_email_keys.sql`, before rolling out the contact-sync correction. The migration removes the surviving `contacts_address_book_primary_email_idx` from `0075`; it leaves local e-mail learning keys and the non-unique lookup intact and does not delete contacts. Two remote contacts sharing an e-mail can then be projected separately. Graph's explicit default collection is synchronized independently rather than equated to an arbitrary discovered folder named “Contacts”; failures of additional folders are reported without suppressing the default read. Existing projection reconciliation is not a remote folder merge or deletion. Without proven provider alias identity, overlapping default/discovered collections remain separate local books. Verification used an isolated PostgreSQL instance: all 527 tests selected by the CI integration step and all 11 tests of the 4.0.4 upgrade gate passed; no live data was migrated.
...
- **One settings surface for calendars and contacts.** Management buttons now open the corresponding Settings section. Calendar navigation separates Accounts from Appearance. Calendar hide/show affects only sidebar presence; event selection stays exclusively in the calendar rail. Local calendar/source lifecycle controls remain available; native provider collection creation/deletion is still restricted rather than simulated locally. Migration `0138_calendar_color_overrides.sql` adds a per-user color presentation preference after `0137_calendar_collection_projection_receipts.sql`; it does not change provider data or permissions. Provider-backed contact imports are explicitly refused; only local books accept local file imports.

- **Grouped v2/v3 management panels.** Calendar account cards now transition to a detail screen instead of rendering list and detail together. Contacts shows an explicit multi-book display selector grouped by connection/account, including partial and empty selection states; the main contact query accepts the union filter while retaining the single-book compatibility parameter. The calendar rail has independent group collapse, group selection and a per-calendar color palette backed by the user presentation preference. These changes were verified with frontend typecheck/build, focused backend routes, migration integrity and i18n tests; full real-provider and live-production visual acceptance remain environment-gated.
...
- **Reply and forward intent remains distinct for retries.** Idempotency fingerprints now include the physical parent, RFC/account fallback and requested reply/reply-all/forward action. Forwarding carries the selected parent too, so Microsoft Graph can use its native `createForward` action after a move rather than creating an unrelated draft, even when the forward has no attachments.
- **Replies resolve their selected parent on the server.** Every reply submission resolves its physical parent and RFC chain server-side. Microsoft Graph additionally resolves the direct or verified legacy native message identity and uses the provider reply action; a missing, cross-account or unusable Graph parent is refused rather than degraded into a new `Re:` message. Single reader, list, swipe and Conversation Reader share the same reply intent builder. Saved drafts retain reply/reply-all intent plus a same-account RFC parent fallback, so reopening after a move can re-resolve the current native parent. A sanitized diagnostic report records reply-resolution mode, chain length and post-ingest thread-match outcomes without message content, addresses or provider IDs. Operator tooling can run a bounded dry-run/write repair for replies missing a Conversation v2 projection: it requires exactly one same-account RFC parent, never uses subject matching, and protects manual locks/overrides while reporting repaired, ambiguous, missing-parent and protected counts. It is not exposed as an end-user route. Validation includes mocked SMTP raw-MIME parsing of a de-duplicated multi-hop `References` chain; it does not replace an OVH live-account acceptance run.
- **Contacts books are grouped by their source account.** The manager separates local, CardDAV, Google and Microsoft sections by the connection/mailbox that owns their books, preventing same-named collections from different accounts appearing to share actions.
- **CardDAV is presented as a connection, not as a selected address book.** Its source controls appear separately in Contacts manager, so selecting a Google or Microsoft book does not imply that it owns CardDAV credentials.
- **New contacts name their destination.** The create form shows a writable address-book selector, including the owning mailbox for provider books. The selected target is captured for the save, so changing a surrounding list filter cannot redirect the write.
- **Provider contact books include their account context.** The address-book read model now returns a provider collection’s connection, provider and mailbox identity. This makes identical book names from different accounts distinguishable; the book manager displays the mailbox and uses that account’s Contacts sync action.
- **Provider account controls use consistent switches.** Calendar and Contacts on Google/Microsoft account cards now use the same accessible switch pattern as other Settings controls. Disabled services use neutral status styling rather than appearing healthy.
- **Disabled providers stop background and manual calls.** Scheduler, push-hint refresh and Google/Microsoft manual contacts and calendar sync now consult the provider/API switches before dispatching an adapter, so disabling an integration also stops syncs for connections authorized earlier. Unit tests cover disabled provider/API policy before scheduler, push-hint, contacts or calendar adapter dispatch.
- **Calendar sources are grouped by category.** The calendar rail now presents My calendars, External, Google, Microsoft and System headings before the individual source/account groups. This changes presentation only: source/account identities and their safe actions remain distinct.
- **External source syncing can be paused.** ICS/webcal and CalDAV source rows now offer Pause/Resume. Pausing cancels future scheduled syncs without removing the source or its imported local calendar projection. The action is available in Settings and the calendar source manager.
- **Calendar preferences and connections are separate.** Settings → Calendar now has distinct Appearance and Calendar subscriptions sections. The latter creates local Inboxora calendars without an account or URL, and manages ICS/webcal, CalDAV and holiday subscriptions. Removing an external source requires confirmation and states that its upstream provider calendar is preserved.
- **Provider calendar collection actions are safely constrained.** Generic rename, color and delete now apply only to local calendars. Provider-backed calendars still support visibility and configured event write-back, but remote calendar lifecycle is deliberately not exposed until an upstream-first, journalled provider operation is implemented; no local projection is deleted as a substitute.
- **Threaded list cards keep a stable representative.** When thread copies share a timestamp, message-row IDs now deterministically break both window and page-order ties. This prevents the visible copy, read state or folder context from changing merely because the database chose a different equal-date row.
- **Infinite scrolling respects navigation boundaries.** An in-flight offset page now uses the same latest-request guard as a list reset. Switching accounts or folders before it returns discards the old response rather than appending it to the new view.
- **Diagnostics identify the loaded pair.** A sanitized diagnostic report now records backend/frontend build SHA, the loaded hashed UI entry asset, current route and applied migration names. It excludes tokens, account identifiers and message content, so it can establish whether a reproduction used matching UI/API/schema revisions.
- **CalDAV connects from Calendar settings.** The credentials form is in Settings → Calendar alongside calendar preferences; the source manager now adds ICS/webcal sources and manages sources/collections after they exist, rather than serving as a CalDAV login form.
- **Provider account cards stay compact.** Google and Microsoft service controls, reconnect/retry and diagnostics are available after opening the specific account for editing rather than expanding every row in the Accounts list. Service enablement is staged until Save, so Cancel does not change it. Native account validation no longer asks for hidden IMAP fields.
...
- **Gmail replies retain the provider conversation.** Reply and Reply All sends pass the stored Gmail `provider_thread_id`, with a safe `gmail:` identity fallback, together with the RFC `In-Reply-To` and `References` headers. Gmail reply drafts include `message.threadId`; new messages do not receive a synthetic thread ID. Microsoft Graph remains on `createReply`/`createReplyAll` → PATCH → send.- **DAV Basic and Digest authentication share one guarded client path.** CardDAV and CalDAV discovery, reads, privilege detection, ETag-protected PUT/DELETE and write-back replay Digest challenges, including Baikal-style MD5/qop=auth, while preserving public-HTTP, private-host and redirect validation. Private plaintext HTTP does not send Basic credentials before a challenge.
