...
- **Reply and forward intent is idempotent and provider-native.** Send idempotency now includes the physical parent, durable RFC/account fallback and requested reply/reply-all/forward action; forwarding preserves the same parent identity so Microsoft Graph can use `createForward` after a move.
