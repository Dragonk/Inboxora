...
- **A bounded orphaned-reply repair is available to operators.** It only follows one same-account RFC parent edge, skips ambiguous/missing/manual-override candidates, supports dry-run, and reports scanned/repaired/ambiguous/missing/protected counters; it never groups by subject.
