...
- **Calendar presentation preferences validate actual source/calendar ownership.** The API applies source collapse and calendar sidebar-hidden preferences only to the requesting user’s current presentation; it keeps selection and event filtering separate. The main sidebar grouping/restore interface remains unfinished until its frontend change is committed.
