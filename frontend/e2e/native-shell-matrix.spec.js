...
for (const [name, grouped, reader] of [['off-off', false, false], ['on-off', true, false], ['off-on', false, true], ['on-on', true, true]]) {
  test(`${name}: native geometry screenshot`, async ({ page, fixtureApi }) => {
    ...
    // Grouped mode renders only the newest parent row until the user expands it.
    await expect(page.locator(`[data-msgid="conversation-gmail-copy-${grouped ? 5 : 3}"]:visible`)).toBeVisible();
