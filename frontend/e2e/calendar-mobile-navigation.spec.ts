import { selectCalendarView, returnToMail } from './navigation.ts';
...
test('mobile calendar source management opens canonical settings', async ({ page, fixtureApi }, testInfo) => {
  test.skip(!['chromium-mobile-390', 'chromium-mobile'].includes(testInfo.project.name), 'mobile source settings');
  await fixtureApi;
  await page.goto('/?list=0&reader=0');
  await page.getByTestId('mobile-topbar-menu').click();
  await page.getByTestId('calendar-nav-mobile').click();
