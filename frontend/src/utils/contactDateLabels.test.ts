...
import { contactDateLabel, localizeContactEvent, localizeContactCalendar } from './contactDateLabels.ts';
...
it('formats a birthday without a year in the selected language', async () => {
  const { formatContactDate } = await import('./contactDateLabels.ts');
