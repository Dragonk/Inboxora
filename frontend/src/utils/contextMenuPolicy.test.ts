...
import { getContextMenuPolicy, resolveContextMenuMessage } from './contextMenuPolicy.ts';
...
test('GTD actions resolve the current row by stable Message-ID', async () => {
  ...
  }, 'gtdSidebar', async (ref: string, accountId: string) => {
...
test('GTD resolution falls back to a legacy row id', async () => {
  const result = await resolveContextMenuMessage(
    ...
    'gtdSidebar',
    async (ref: string, accountId: string) => ({ id: ref, accountId }),
  );
