...
import { calendarSyncWarning } from './calendarSyncWarning.ts';
...
it('handles structured warnings and malformed optional samples', () => {
  ...
  const result = calendarSyncWarning(JSON.stringify({ code: 'unsupported_events', count: 2, samples: [{ uid: 'a', reason: 'invalid' }] }));
  assert.ok(result);
  assert.equal(result.details, 'a: invalid');
});
