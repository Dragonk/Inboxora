...
function worker(matchAll: () => Promise<unknown>): {
  ...
  shown: unknown[][];
  sent: Array<[string, { headers: Record<string, string> }]>;
} {
  const listeners: Record<string, (event: unknown) => void> = {};
  const shown: unknown[][] = [];
  const sent: Array<[string, { headers: Record<string, string> }]> = [];
...
test('push displays with no app window and even when enumerating clients fails', async () => {
  ...
  let done;
  listeners.push({ data: { json: () => ({ title: 'Synthetic mail', body: 'Details' }) }, waitUntil: (p: unknown) => { done = p; } });
  await done;
...
test('push notifies open app windows so a stale websocket is not the only refresh path', async () => {
  ...
  const { listeners } = worker(async () => [{ postMessage: (data: { type?: string; [key: string]: unknown }) => messages.push(data) }]);
  let done;
  listeners.push({ data: { json: () => ({ title: 'Synthetic mail' }) }, waitUntil: (p: unknown) => { done = p; } });
...
test('subscription renewal persists the new endpoint without an open settings page', async () => {
  ...
  let done;
  listeners.pushsubscriptionchange({ newSubscription: { toJSON: () => ({ endpoint: 'https://push.example.test/new' }) }, waitUntil: (p: unknown) => { done = p; } });
  await done;
