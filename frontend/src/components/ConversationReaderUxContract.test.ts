...
describe('conversation reader final UX contract', () => {
  it('renders no body for collapsed cards and keeps quote folding inside expanded content', () => {
    const message = read('ConversationMessage.tsx'); const detail = read('MessageDetailContent.tsx');
  ...
  it('auto-reads only the selected target and preserves sibling card read state', () => {
    const reader = read('ConversationReader.tsx');
    const message = read('ConversationMessage.tsx');
  ...
  it('positions each navigation target once in the reader scroll container', () => {
    ...
    const message = read('ConversationMessage.tsx');
    const alignment = read('readerScrollAlignment.ts');
    assert.match(reader, /data-conversation-message-scroll-anchor/);
    assert.match(message, /scrollAnchorId=\{copy\.id\}/);
    assert.match(message, /data-conversation-message-header=\{copy\.id/);
    assert.match(message, /<MessageToolbar/);
    assert.match(reader, /alignReaderHeader\(reader, anchor\)/);
    assert.match(reader, /onInitialBodyLayout/);
    assert.match(reader, /bodyReady/);
    assert.match(reader, /completedNavigationRef\.current\.add\(navigationKey\)/);
    assert.doesNotMatch(reader, /ResizeObserver/);
    const body = read('MessageBodyRenderer.tsx');
    assert.match(body, /onInitialLayoutReady/);
  ...
  it('keeps forwarded-message protection in the shared folding pipeline', () => {
    const folding = read('messageQuoteFolding.ts');
  ...
  it('uses a white light-theme body token without forcing dark themes to white', () => {
    ...
    const message = read('MessageDetailContent.tsx');
