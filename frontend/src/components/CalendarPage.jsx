...
export default function CalendarPage({ isActive = true }) {
  ...
  return <div data-testid="calendar-page" className="calendar-page" style={{ ...page, ...(isMobile ? mobilePage : {}) }}>
    <header style={{ ...header, ...(isMobile ? mobileHeader(mobileNavigationPosition) : calHead) }}>
      ...
      <div style={{ ...heading, ...(isMobile ? mobileHeading : {}) }}><h1 style={headingTitle}>{title}</h1></div>
...
function CalendarGrid({ days, events, view, anchor, isMobile, locale, openCreate, openEdit, openContextMenu, t, calendarWorkHoursStart, calendarWorkHoursEnd }) {
  ...
  return <div data-testid="calendar-grid" style={{ ...calendarSurface, flex: 1, minWidth: 0 }}>
    ...
    <div data-testid="calendar-month-grid" style={{ ...dayGrid, gridTemplateColumns: `repeat(7, minmax(${isMobile && !month ? 112 : 0}px, 1fr))`, gridAutoRows: 'minmax(108px, 1fr)', gap: 1, background: 'var(--border-subtle)', borderTop: '1px solid var(--border-subtle)' }}>
      {days.map(day => {
        const inMonth = day.getMonth() === anchor.getMonth(); const dayEvents = eventsForDay(events, day); const visibleDayEvents = dayEvents.slice(0, 3); const hiddenCount = dayEvents.length - visibleDayEvents.length;
        return <section key={day.toDateString()} className="cal-cell" onDoubleClick={() => openCreate(day)} style={{ ...monthCell, ...(isWeekend(day) ? weekendCell : {}), ...(inMonth ? {} : outCell) }}>
          <span style={{ ...dateChip, ...(isToday(day) ? dateChipToday : {}) }}>{day.getDate()}</span>
          ...
          })}{hiddenCount > 0 && <span style={eventMoreChip}>{t('calendar.moreEvents', { n: hiddenCount })}</span>}</div>
        </section>;
      })}
    </div>
  </div>;
...
function TimeGrid({ days, events, view, isMobile, locale, openCreate, openEdit, openContextMenu, t, calendarWorkHoursStart, calendarWorkHoursEnd }) {
  ...
  return <div data-testid="calendar-grid" style={{ ...calendarSurface, flex: 1, minWidth: 0, overflowX: 'auto', overflowY: 'hidden' }}>
    <div data-testid="calendar-time-grid-scroll" ref={scroller} style={{ width: isMobile ? 52 + days.length * 150 : '100%', overflowY: 'auto', overflowX: 'visible', maxHeight: 'min(72vh, 760px)' }}>
...
const calHead = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap', padding: '14px 18px', borderBottom: '1px solid var(--border-subtle)', background: 'var(--bg-secondary)', marginBottom: 0 };
const headingTitle = { margin: 0, fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' };
...
const eventMoreChip = { display: 'block', width: '100%', textAlign: 'left', border: 0, borderRadius: 4, padding: '2px 6px', fontSize: 10, fontFamily: 'var(--font-mono, ui-monospace, monospace)', color: 'var(--text-secondary)', background: 'var(--bg-tertiary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', cursor: 'default' };
