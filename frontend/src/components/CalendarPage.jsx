...
function CalendarGrid({ days, events, view, anchor, isMobile, openCreate, openEdit, t }) {
  ...
  return <div data-testid="calendar-grid" style={{ ...calendarSurface, flex: 1, minWidth: 0 }}>
    <div style={{ ...dayGrid, gridTemplateColumns: `repeat(${month ? 7 : days.length}, minmax(${isMobile ? 112 : 0}px, 1fr))` }}>
...
const errorStyle = { marginBottom: 12, padding: 10, borderRadius: 7, color: 'var(--red)', background: 'var(--red-dim, #fee2e2)' }; const calendarSurface = { overflowX: 'auto', border: '1px solid var(--border)', borderRadius: 12, background: 'var(--bg-secondary)', boxShadow: '0 6px 24px rgba(0,0,0,.08)' }; const dayGrid = { display: 'grid', minWidth: 0 }; const dayHeader = { position: 'sticky', top: 0, zIndex: 1, display: 'flex', justifyContent: 'space-between', padding: '9px 10px', borderBottom: '1px solid var(--border)', color: 'var(--text-secondary)', background: 'var(--bg-secondary)', fontSize: 12 }; const todayHeader = { color: 'var(--accent)' }; const dayCell = { padding: 7, borderRight: '1px solid var(--border)', borderBottom: '1px solid var(--border)', boxSizing: 'border-box' }; const todayCell = { background: 'color-mix(in srgb, var(--accent) 5%, transparent)' }; const dateChip = { fontSize: 12, color: 'var(--text-secondary)', marginBottom: 6 }; const eventStack = { display: 'grid', gap: 4 }; const eventCard = { display: 'grid', gap: 2, width: '100%', textAlign: 'left', border: 0, borderRadius: 6, padding: '5px 7px', color: 'white', fontSize: 12, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' };
