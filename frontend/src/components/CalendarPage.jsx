...
function CalendarGrid({ days, events, view, anchor, isMobile, locale, openCreate, openEdit, openContextMenu, t, calendarWorkHoursStart, calendarWorkHoursEnd }) {
  ...
  return <div data-testid="calendar-grid" style={{ ...calendarSurface, flex: 1, minWidth: 0 }}>
    <div data-testid={month ? 'calendar-month-grid' : undefined} style={{ ...dayGrid, gridTemplateColumns: `repeat(${month ? 7 : days.length}, minmax(${isMobile && !month ? 112 : 0}px, 1fr))` }}>
      {days.slice(0, month ? 7 : days.length).map(day => <div key={`header-${day.toISOString()}`} style={dayHeader}><span data-testid="calendar-weekday" style={dayHeaderWeekday}>{day.toLocaleDateString(locale, { weekday: 'short' })}</span><strong style={{ ...dayHeaderDay, ...(isToday(day) ? todayDayChip : {}) }}>{day.getDate()}</strong></div>)}
      {days.map(day => {
        const inMonth = day.getMonth() === anchor.getMonth(); const dayEvents = eventsForDay(events, day);
        return <section key={day.toDateString()} onDoubleClick={() => openCreate(day)} style={{ ...dayCell, minHeight: month ? 132 : 580, opacity: month && !inMonth ? .5 : 1, ...(month && isWeekend(day) ? weekendCell : {}), ...(isToday(day) ? todayCell : {}) }}>
          {month && <div style={{ ...dateChip, ...(isToday(day) ? dateChipToday : {}) }}>{day.getDate()}</div>}
          <div style={eventStack}>{dayEvents.map(event => {
            ...
            return <div key={event.id} style={eventRow}>
              <button className="cal-ev" onClick={() => writable ? openEdit(event) : null} onContextMenu={event => { event.preventDefault(); showMenu(event); }} onKeyDown={invokeMenu} title={event.read_only ? t('calendar.readOnly') : t('calendar.edit')} style={{ ...eventCard, background: event.calendar_color || 'var(--accent)', cursor: writable ? 'pointer' : 'default' }}>{!(event.all_day || event.allDay) && <strong style={eventCardTime}>{eventTime(event)}</strong>}<span>{month ? event.summary || t('calendar.untitled') : `${eventTime(event)}  ${event.summary || t('calendar.untitled')}`}</span>{!month && event.location && <small>{event.location}</small>}</button>
...
function EventDialog({ form, calendars, accounts, isMobile, saving, onChange, onAllDayChange, onSave, onDelete, onClose, t }) {
  ...
  return <div role="dialog" aria-modal="true" aria-label={form.mode === 'edit' ? t('calendar.editEvent') : t('calendar.newEvent')} style={overlay}><div data-testid="calendar-event-dialog" style={{ ...dialog, ...(isMobile ? mobileDialog : {}) }}>
    <header style={dialogHeader}><h2 style={{ margin: 0 }}>{form.mode === 'edit' ? t('calendar.editEvent') : t('calendar.newEvent')}</h2><button className="cal-btn" aria-label={t('calendar.close')} onClick={onClose} style={iconButton}>×</button></header>
    ...
    <footer style={dialogFooter}><div>{form.mode === 'edit' && <button className="cal-btn" disabled={saving} onClick={onDelete} style={dangerButton}>{t('calendar.delete')}</button>}</div><div style={{ display: 'flex', gap: 8 }}><button className="cal-btn" disabled={saving} onClick={onClose} style={secondaryButton}>{t('calendar.cancel')}</button><button className="cal-btn" disabled={saving} onClick={onSave} style={primaryButton}>{saving ? t('calendar.saving') : form.invitationError ? t('calendar.retrySave', 'Retry save') : t('calendar.save')}</button></div></footer>
  </div></div>;
}
...
const errorStyle = { marginBottom: 12, padding: 10, borderRadius: 7, color: 'var(--red)', background: 'var(--red-dim, #fee2e2)' }; const calendarSurface = { overflowX: 'auto', border: '1px solid var(--border)', borderRadius: 12, background: 'var(--bg-secondary)', boxShadow: '0 6px 24px rgba(0,0,0,.08)' }; const dayGrid = { display: 'grid', minWidth: 0 }; const dayHeader = { position: 'sticky', top: 0, zIndex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, minWidth: 0, padding: '7px 6px', borderBottom: '1px solid var(--border)', background: 'var(--bg-secondary)' }; const dayHeaderWeekday = { fontFamily: 'var(--font-mono, ui-monospace, monospace)', fontSize: 9.5, letterSpacing: '0.07em', textTransform: 'uppercase', color: 'var(--text-tertiary)' }; const dayHeaderDay = { fontFamily: 'var(--font-mono, ui-monospace, monospace)', fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', lineHeight: 1 }; const todayDayChip = { background: 'var(--accent)', color: 'var(--accent-text)', width: 26, height: 26, borderRadius: '50%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }; const dayCell = { minWidth: 0, padding: 7, borderRight: '1px solid var(--border)', borderBottom: '1px solid var(--border)', boxSizing: 'border-box' }; const todayCell = { background: 'color-mix(in srgb, var(--accent) 5%, transparent)' }; const weekendCell = { background: 'color-mix(in srgb, var(--bg-secondary) 55%, var(--bg-primary))' }; const dateChip = { fontFamily: 'var(--font-mono, ui-monospace, monospace)', fontSize: 11, color: 'var(--text-secondary)', marginBottom: 6, width: 20, height: 20, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }; const dateChipToday = { background: 'var(--accent)', color: 'var(--accent-text)', fontWeight: 600 }; const eventStack = { display: 'grid', minWidth: 0, gap: 4 }; const eventRow = { display: 'flex', minWidth: 0, gap: 2 }; const eventCard = { display: 'grid', minWidth: 0, gap: 2, flex: 1, width: '100%', textAlign: 'left', border: 0, borderRadius: 4, padding: '2px 6px', color: 'white', fontSize: 11, lineHeight: 1.5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }; const eventCardTime = { fontFamily: 'var(--font-mono, ui-monospace, monospace)', fontSize: 9.5, fontWeight: 400, opacity: .85, marginRight: 4 }; const eventActionButton = { flexShrink: 0, width: 44, minWidth: 44, height: 44, minHeight: 44, border: 0, borderRadius: 6, background: 'var(--bg-tertiary)', color: 'var(--text-primary)', cursor: 'pointer', fontSize: 18, lineHeight: 1 };
const timeAxisHeader = { borderRight: '1px solid var(--border)' }; const allDayLabel = { padding: '8px 6px', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono, ui-monospace, monospace)', fontSize: 9.5, borderRight: '1px solid var(--border)' }; const allDayCell = { minHeight: 30, padding: 2, borderRight: '1px solid var(--border)', display: 'grid', gap: 2, alignContent: 'start' }; const timeAxis = { position: 'relative', height: 1440, borderRight: '1px solid var(--border)', color: 'var(--text-tertiary)', fontSize: 10 }; const timeAxisSpan = { display: 'block', height: 60, padding: '3px 6px', boxSizing: 'border-box', textAlign: 'right', fontFamily: 'var(--font-mono, ui-monospace, monospace)', fontSize: 9.5 }; const timeColumn = { position: 'relative', height: 1440, borderRight: '1px solid var(--border)', backgroundImage: 'repeating-linear-gradient(to bottom, transparent 0, transparent 59px, var(--border) 59px, var(--border) 60px)' }; const weekendColumn = { background: 'color-mix(in srgb, var(--bg-secondary) 55%, var(--bg-primary))' }; const timedEvent = { position: 'absolute', zIndex: 1, margin: 0, overflow: 'hidden', border: 0, borderRadius: 4, padding: '2px 6px', color: 'white', textAlign: 'left', fontSize: 11, lineHeight: 1.45, boxSizing: 'border-box' }; const timedEventTime = { fontFamily: 'var(--font-mono, ui-monospace, monospace)', fontSize: 9.5, fontWeight: 400, opacity: .85, marginRight: 2 };
