      ...
      <div style={{ width: isMobile ? 72 + days.length * 150 : '100%', minWidth: isMobile ? 72 + days.length * 150 : 0 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: columns }}>
          ...
          {days.map(day => {
            ...
            return <div key={day.toDateString()} onDoubleClick={() => openCreate(day)} style={timeColumn}>{Array.from({ length: 24 }, (_, hour) => <i key={hour} style={{ top: hour * 60 }} />)}<div aria-label={`${t('calendar.workHoursStart', 'Working hours start')} ${calendarWorkHoursStart} – ${t('calendar.workHoursEnd', 'Working hours end')} ${calendarWorkHoursEnd}`} data-testid="calendar-work-hours-boundary" style={{ ...workHoursBoundary, top: workHours.start, height: Math.max(0, workHours.end - workHours.start) }} />{timed.map(({ event, geometry, column, columns: count }) => <div key={event.id} style={{ ...timedEvent, top: geometry.start, height: Math.max(18, geometry.end - geometry.start), left: `calc(${column * 100 / count}% + 3px)`, width: `calc(${100 / count}% - 6px)`, padding: 0, display: 'flex', overflow: 'visible' }}><button onClick={() => !event.read_only && event.source === 'local' && openEdit(event)} onContextMenu={keyboardEvent => { keyboardEvent.preventDefault(); showMenu(event, keyboardEvent); }} onKeyDown={keyboardEvent => invokeMenu(event, keyboardEvent)} title={event.read_only ? t('calendar.readOnly') : t('calendar.edit')} style={{ ...timedEvent, position: 'absolute', inset: 0, width: '100%', height: '100%', background: event.calendar_color || 'var(--accent)', cursor: event.read_only || event.source !== 'local' ? 'default' : 'pointer' }}><strong>{eventTime(event)}</strong> {event.summary || t('calendar.untitled')}</button>{isMobile && <button type="button" data-testid="calendar-event-actions" aria-label={t('calendar.eventActions', 'Event actions')} onClick={target => showMenu(event, target)} style={{ ...eventActionButton, position: 'absolute', top: 0, right: 0, zIndex: 2 }}>⋮</button>}</div>)}</div>;
          })}
        </div>
      </div>
    </div>
