...
describe('native conversation 2x2 contract', () => {
  it('keeps one native list and one native pane for every preference pair', () => {
    ...
    assert.match(app, /conversationReaderViewEnabled\s*&&\s*\(conversationId\s*\|\|\s*nativeThreadId\)\s*\?\s*['"]conversation['"]\s*:\s*['"]single['"]/);
  ...
  it('resolves every selected physical copy through the CE identity endpoint (OFF/ON blocker)', () => {
    const app = read('MailApp.tsx');
    assert.match(app, /conversationApi\.resolveMessage\(selectedMessageId, selected\?\.account_id \|\| null\)/);
    assert.match(app, /setSelectedConversationCopy\(\{\s*id: selected\?\.id \|\| selectedMessageId/);
    assert.match(app, /setTargetLogicalMessageId\(selected\?\.message_id \|\| selectedMessageId\)/);
    ...
    assert.match(app, /id: resolved\.physical_copy_id \|\| resolved\.id \|\| selected\?\.id \|\| selectedMessageId/);
    assert.match(app, /accountId: resolved\.account_id \|\| resolved\.accountId \|\| selected\?\.account_id \|\| null/);
    ...
    assert.match(app, /mode=\{conversationReaderViewEnabled && \(conversationId \|\| nativeThreadId\) \? 'conversation' : 'single'\}/);
    const pane = read('MessagePane.tsx');
